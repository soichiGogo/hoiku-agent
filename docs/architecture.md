...
## 参照ポリシーの agentic 取得（2026-07-11・2026-07-12簡素化）

育つ指針の PolicyBook は「何を参照すべきか」を専用カード種別・構造化トグルUIで管理せず、他の指針カードと同じ自然文の1枚（PolicyScope 別）として持つ（例:「保育経過記録の作成では、対象期間の保育日誌と、前回までの保育経過記録すべてを参照する」）。InstructionProvider は scope の guideline カード（参照方針もその1枚）をそのまま前置し、参照本文自体は固定注入しない。author は自然文の案内に従い `fetch_reference(source)`（`ReferenceSource` の閉じた列挙値）で session state に seed 済みの候補を選択取得し、reviewer は `reference_manifest` で取得実績を検証して必要なら再取得する。

構造化カード種別（旧 `kind=reference_policy`）・チェックボックス式トグル（旧 `ReferenceRule`）・専用編集エンドポイント（旧 `PATCH /api/policy/reference`）は撤去した。参照方針の追加/改訂は改善エージェントの提案→比較相談→即反映フロー（他の指針カードと完全に同じUX）で行う＝ツール呼び出し精度が構造化トグルより多少落ちることは、UXの単純さのために許容する（ユーザー判断・2026-07-12）。

pipeline 常設の digest prep は撤去し `[authoring_loop → finalize]` とした。digest 化とクラス月案の児童別未反映境界は `harness/reference.py` から既存 aggregate/record_store を呼ぶ決定的実装に残す（不変）。Web/scripts の seed API・state key・workspace/PII 境界は不変。main の確立済み baseline は維持し、本変更後の母集団はマージ後に strict・完全採点で意図的に再採点して通常PRで更新する。

improver の PolicyBook は呼び出し側で束縛する（reference_policy 固有ではなく全ての指針カード操作に共通の認可境界）。Web は署名付き session から共有 `web/workspace.py` で解決した `workspace:{workspace_id}`、CLI は default を使い、`build_policy_tools` のクロージャへ閉じ込める。book_id はLLMのツール引数にせず、Webの保留セッション再開時も開始時workspaceとの一致を検査する。
...
| `harness/pipeline.py` | `build_authoring_loop` / `ApprovalGate` / `FinalizeAgent`(kind) / `is_approved` | 作成パイプラインの共用機構。`authoring_loop → finalize` の順序を保証し、生成終了時には Memory Bank へ書かない。 |
| `harness/memory_writeback.py` | `approved_memory_facts` / `persist_approved_facts` | 保存済み現行版から子どもの事実欄だけを抽出し、Web `/api/records/approve` が Memory Bank 書き戻し成功後に承認済み化する。接続済み失敗は fail-closed、未接続は降格（§9）。 |
| `harness/monthly.py` | `build_monthly_pipeline` | `[authoring_loop → finalize(monthly)]`。前月日誌候補は state に seed し、author が fetch_reference で取得 |
| `harness/class_monthly.py` | `build_class_monthly_pipeline` | `[authoring_loop → finalize(class_monthly)]`。3系統＋在籍児名簿（class_roster）の候補取得と未反映境界は harness/reference が担当 |
| `harness/child_record.py` | `build_child_record_pipeline` | `[authoring_loop → finalize(child_record)]`。期間日誌と前回記録は fetch_reference で取得 |
| `harness/child_record_period.py` | `child_record_periods` / `child_record_period_for` / `parse_child_record_period` / `child_record_period_options` | 保育経過記録の対象期間を年度4期・各3か月（4〜6月／7〜9月／10〜12月／1〜3月）へ固定する決定的実体。確定検査・保存時正準化・帳票列割当・Web選択肢が共用する |
| `harness/youroku.py` | `build_nursery_record_pipeline` | `[authoring_loop → finalize(nursery_record)]`。全期の保育経過記録を fetch_reference で取得し、日誌は加えない |
...
| `harness/record_store.py` | `Workspace` / `touch_user` / `save_document` / `approve_document` / `list_documents` / `get_document` / `purge_workspace_data` / `request_workspace_deletion` / `process_due_deletion_requests` ほか | **Google user ごとの個人 workspace が認可境界**（migration 0011）。書類・園児・クラス・フィードバック・アーカイブ読取/集積を `workspace_id` で必ず絞る。既存データは所有者を推測せず「既存データ」workspace へ退避し、新規ログインからは見せない。`touch_user` は新規 workspace を作った呼び出しだけ `workspace_created=True` を返す（初回ログイン auto-seed の単発トリガ）。`purge_workspace_data` は workspace のデータ行だけを即時消去（User/Workspace/利用枠は残す＝「データを初期化」の消去段。消去順＝`_purge_workspace_rows` を削除依頼処理と共用）。削除依頼（30日遅延）は API 温存・UI からは外し問い合わせベース。**登場する子ども索引（2026-07-12）**＝`documents.mentioned_children`（JSON配列＝表示名。migration 0014）を `save_document` が毎回 `_mentioned_children()` の出力で現行版に同期する（日誌/クラス月案の複数児・保育経過記録/保育要録/月案の単一児いずれも同じ列で引ける＝検索キーの列昇格。射影テーブルは作らない・本文JSONがSSOTの既存方針のまま）。`_doc_view` の `children` フィールドとして返し、「書類を見る」タブの対象児フィルタ（`records.js`）が読む。 |
| `harness/demo_seed.py`＋`demo_seed_data.py` | `validate_all` / `seed_workspace` / `reset_workspace`／データ部＝`ROSTER`（仮名10人）・`CLASSES`・`GRADUATE`・書類167件・`UNAPPROVED`・`INCOMPLETE_DATES` | **デモ用デフォルト seed**（実在しない仮名のみ・§14・物語時刻＝FY2026 の7月中旬）。初回ログイン（`web/workspace.py` の `provision_user`）で新規 workspace へ、クラス2（ひよこ組=0-2／あおぞら組=3-5・在籍児は誕生日→`roster_age_band` で割当）＋名簿10人＋確定書類チェーン計167件（**保育日誌138＝2026-04-01〜07-10 の平日毎日×2クラス・週テーマ×子ども別観察プールの決定的ローテ合成・age_months は生年月日から機械計算**／クラス月案8／**保育経過記録20＝年度4期・各3か月の閉じた期間を使う前年度 FY2025〔「前回まで」還流と年度跨ぎ集積の素・今年度第1期はあえて未作成＝溜まった日誌から作るデモの主戦場〕**／要録1＝卒園児）を冪等投入（種別×児×期間で既存スキップ）。`UNAPPROVED`（最新クラス月案）と`INCOMPLETE_DATES`（直近2日の日誌＝評価未記入の記入導線デモ・validate 除外＋承認せず finalized）だけ未承認で残し承認フローを体感できる。「データを初期化する」（web `/api/account/reset`）＝`reset_workspace`（purge→再 seed・ログイン継続。毎回使える＝一回限りの開始導線ではない）。CLI（`scripts/seed_children.py`・`scripts/seed_documents.py`）は薄いラッパ。投入は `finalize_entry`→`save_document`/`approve_document` の正規経路（Memory Bank へは書かない・未接続 skipped 降格）。整合は `tests/test_harness/test_demo_seed.py` が検算 |
...
| 対象 | 置き場 | 参照 |
|---|---|---|
| 子ども別 長期メモリ | Agent Engine Memory Bank（repo外） | 読み＝`recall_child_history`／書き戻し＝Webの `/api/records/approve`→`approved_memory_facts`→`persist_approved_facts`。DBの保存済み現行版だけを対象に、同期成功後に承認する。`documents.memory_synced_version_id`（migration 0013）で版単位に二重投入を防ぎ、編集後の新版は再承認で再同期。配線は `agentengine://<id>`。未設定は降格 |
...
観察メモ＋ state["doc_type"]（月案/クラス月案/保育経過記録/保育要録）＝AI 生成書類
  └─ harness: DocTypeRouter (root_agent) … doc_type で各パイプラインを決定的に振り分け（既定＝クラス月案・§10/§19）
       │
       └─[月案]─ monthly_plan_pipeline (SequentialAgent)  ※ author が fetch_reference で前月日誌候補を取得
       ...
       └─[クラス月案]─ class_monthly_pipeline (SequentialAgent・§18・園の実様式・依存モデル 2026-07)
            │                              ※ author が fetch_reference で3系統＋在籍児名簿（class_roster）を取得。児童別境界は harness/reference
            ├─ authoring_loop （共用: class_monthly_author→reviewer→approval_gate を巡回・再作成）
            │    └─ class_monthly_author (LlmAgent) … クラス全体のねらい・区分×領域グリッド（養護2本柱＋教育5領域）・0–2 は在籍児名簿
            │                                        （class_roster・名簿なしは蓄積の登場児）ぶんの個人目標を生成（評価系欄は月末記入＝AI 非生成）→ state["draft"]
       ...
       └─[保育経過記録]─ child_record_pipeline (SequentialAgent・§19・依存モデル 2026-07)
            │                              ※ author が fetch_reference で期間日誌と前回記録を取得
       ...
       └─[保育要録]─ nursery_record_pipeline (SequentialAgent・§19・L4・年長のみ・依存モデル 2026-07＝日誌は足さない)
            │                              ※ author が fetch_reference で全期の保育経過記録を取得（日誌は足さない）
       ...
       │
       └─生成終了（この時点では保育士未承認のためMemory Bankへは書かない）
保育士が保存済み現行版を確認→ `/api/records/approve` → 子ども別factをMemory Bankへ同期
       └─同期成功後だけ document.status=approved＋memory_synced_version_idを記録（§9/§13）
...
- **保育日誌の手入力化＋AI 生成の退役＋クラス概念（2026-07-06・ヒアリング反映）**：日誌は「AI が観察メモから
  ...
  既定を保育日誌→クラス月案へ・日誌 eval 16件を撤去〔母集団変更後に baseline は完全採点で確立済み〕・
  共用機構の E2E〔`test_pipeline_e2e`〕を保育経過記録パイプラインへ再ポイント）。共用機構
  （`build_authoring_loop`/`FinalizeAgent`）と日誌の決定的検査・整形は不変で流用。手入力日誌も
  `/api/records/approve` からMemory Bankへ反映するため、ADKセッション有無に依存しない。④ **校正AI（日本語チェック・言い換え提案）**：手入力後
...
- **doc_type 分岐 ＋ 月案パス ＋ L2 還流**：`DocTypeRouter`（root_agent）が doc_type で月案/クラス月案/保育経過記録/保育要録を振り分け（既定＝クラス月案・**保育日誌は手入力＝ルータ外**）、
  月案は `monthly_author` が fetch_reference で前月日誌を取得・要約・ねらい化 →
  `validate_monthly_fields`/`write_monthly_draft` で確定（§3/§4/§10）。`MonthlyPlan` スキーマ・
...
- **クラス月案パス（園の実様式＝月間指導計画・L2 還流・§18）**：園が実際に使うクラス月案フォーム
  ...
  個別月案（1児）と別 doc_type＝**文書の年齢帯単位**。入力は依存モデル 2026-07 の3系統＋在籍児名簿（class_roster＝クラス・園児マスタ・0–2 個人目標の対象の与件）（クラス児童の
  保育経過記録すべて／それまでのクラス月案すべて／経過記録に未反映の期間の日誌）を
  fetch_reference で選択取得（児童別境界は harness/reference）→ `class_monthly_author`（クラス全体の
  ねらい・区分×領域グリッド・0–2 は在籍児名簿（class_roster・名簿なしは蓄積の登場児）ぶんの個人目標を生成／評価系欄は月末記入＝AI 非生成）→
...
- **保育経過記録パス ＋ L3 還流（§19）**：`ChildRecord`/`DevelopmentNote` スキーマ（期・発達の経過＝領域別叙述・
  ...
  生成対象に。原簿・発達チェックリストは AI 外）。期間日誌＋**前回までの保育経過記録すべて**を
  fetch_reference で選択取得（作成対象の期は seed 側で除外・依存モデル 2026-07）→
  ...
  デモ入口＝`scripts/run_child_record.py`。**対象期間は年度4期・各3か月固定**（4〜6月／7〜9月／10〜12月／1〜3月）。
  決定ロジックは `harness.child_record_period`、Web は `/api/config` の閉じた選択肢を単一セレクタで描く。
- **保育要録パス ＋ L4 還流（§19・集積階層の最終段）**：`NurseryRecord` スキーマ（全国統一様式の「保育に関する
  ...
  L4 は **日誌でなくそれまでの保育経過記録すべて（全期・依存モデル 2026-07＝日誌は足さない）**を
  fetch_reference で取得（集計実体は `aggregate.child_record_digest`）→ `nursery_record_author`（**小学校引継ぎ＝開示前提・
...
- **指針と参照方針の提示（§5・2026-07-12簡素化）**：InstructionProvider は scope に応じた guideline
  （参照方針も自然文の1枚として含む）をそのまま提示する。参照本文は author/reviewer が fetch_reference
  で取得し、manifest を共有する。
- **eval 互換（§12）**：pipeline の先頭 content 段は author。非 LLM の参照集計段を置かないため、
  invocation は author→reviewer→gate→finalize の同一 invocation_id で追跡できる。
...
- **決定論E2E（結合テスト）**：`tests/test_e2e/`。`FakeLlm` 注入で日誌/月案/保育経過記録パイプラインを実 ADK ランタイムに
  ...
  HITL 不発火・**L2/L3 還流・ルータ分岐（日誌/月案/保育経過記録）**を creds 不要・決定的に検証。承認→Memory Bankは `tests/test_harness/test_memory_writeback.py`（fact変換）＋`tests/test_web.py`（保存→承認・同期失敗・版冪等）で検証。起動は `/e2e` skill。
- **Memory Bank 配線（読み＋書き戻し）＋ 真の承認ゲート**：`config.memory_service_uri`（`agentengine://<id>`）→
  入口 `server.py` はADK本体へURIを渡し、Web承認APIには同じAgent Engine設定の
  `VertexAiMemoryBankService` を注入（自前Runnerなし）。読み＝`recall_child_history`、書き戻し＝
  `/api/records/approve`。保存済み現行版を型復元→子どもの事実だけをconsolidate（完了待ち）→成功後にapproved＋
  同期版IDを同一DB更新する。接続済み障害は503で承認保留、同じ版の再承認は二重投入しない（§9/§13）。
...
- **eval ゲートの本採点（3軸 rubric 配線）**：`eval/test_config.json` が ADK ネイティブの
  ...
  judge は3回採点を多数決（ADK既定5回に対するコスト折衷）。`eval/run_gate.py` が rubric 採点 →
  **全ケース×全6 rubric coverage 検査** → `aggregate_rubric_scores` → `eval/gate_policy.json` の軸/ケースfloor →
  `decide_gate`（main 比の非劣化マージン内＋must_fix 0）で passed を返す。マージンは
  `gate_policy.json` の0.05（9ケース×3軸＝27セルで1セル差≈0.037のみ許容、2セル差≈0.074は拒否）。
  軸/ケースfloorは別途必須なので局所劣化を平均で隠さない。採点不能・rubric欠落・baseline未確立は
  **passed=None、CIの `--strict` は非0終了**（GitHubのskip=successへ逃がさない）。判定式の純関数は
  `tests/test_eval_gate.py` で LLM 非依存に検証。過去には creds 有で
  ...
  rubric は judge のechoテキストを完全一致でIDへ戻すADK仕様なので、text_propertyを意味を保った短い固定文へ整理。
  ADK 2.3既定parserが複数行Rationaleを落とす実機事象には、外部packageを変更せず `run_gate` の専用metric registryへ
  **rubric ID marker正規化＋複数行parser adapter**を注入。それでも欠落した場合はcoverage不足として赤にし、
  presentのみの平均で緑にしない。ケース別スコア・judge理由は
  `--output` のJSONをActions artifactとして30日保存する。
- **main 比 baseline の保存（committed）**：`eval/baseline.json` に main の eval 平均を保存し、`run_gate` が
  既定で読み PR の非劣化比較に使う。不在・壊れ・mean=nullは判定不能（strictで赤）。**nightlyの自動更新・
  botコミットは廃止**し、`--update-baseline` がcoverage 100%・floor達成・must_fix 0のときだけ書く。
  baseline変更は通常PRでレビューする。PR CIはbase SHAからbaselineを抽出して `--baseline-path` へ渡すため、
  候補PR自身が基準を下げてすり抜けられず、劣化後のmainにも自動追随しない。
...
- **配信（層A）**：`Dockerfile`＋`.dockerignore`（`uvicorn server:app`・scale-to-zero。指針ファイル＋`eval/baseline.json` のみ同梱）、
  ...
  `roles/secretmanager.secretAccessor`）、`.github/workflows/eval-gate.yml`（関連pathのPR＋週次＋手動。
  Terraform管理の専用 `eval-runner` SA＝`roles/aiplatform.user` のみをWIFで借用しstrict採点）。
  決定論 CI（`ci.yml`）は従来どおり毎PR・creds 不要。
...
- **保育士向け配布 UI（`web/`・B-full）**：`server.py` が `register_web_ui(app)` で `get_fast_api_app` に同居させる。
  ...
  **表示に必要な分だけ読む**＝折りたたみ既定で展開したフォルダの DOM のみ都度生成・重い本文はファイルを開いた分だけ取得。読取なので非ゲート・未接続/不在は正直に降格。前月・期間の集計など参照データの点検にも使える）。日誌/月案/保育経過記録/保育要録はフロントが ADK ネイティブ REST
  （`/run_sse`・session 作成で月案/保育経過記録/要録の seed・`function_response` で HITL 再開）を直接駆動（自前 Runner なし＝§9。承認はADK state PATCHでなく `/api/records/approve`。クラス月案は seed 3系統＋在籍児名簿＝`GET /api/records/class-monthly-seed`・保育経過記録は期間日誌＋前回までの記録〔`exclude_period`〕・要録は年度指定＋それまでの保育経過記録すべて＝`GET /api/records/child-record-entries`）。
  **確定下書きは標準様式の見た目の編集フォーム（`docedit.js`）で保育士が欄ごとに自由に編集**でき（出欠/個別記録/教育ねらいは
  ...
  `/api/finalize-edit`（harness の `finalize_entry` 中継）で再 validate/整形→state へ反映、保存済み版のMemory同期成功後に承認して公式記録にロック。
...
- **Google Sign-In（Phase 3・2026-07-11）**：`/` は案内画面を返し、Google Identity Services の**公式ボタン**を押したときだけ
  Google の選択・同意画面（popup）を開く。ID token は案内画面と同一Originの fetch（`POST /auth/google`）で送り、
  `web/auth.py` が Google の公開鍵で署名・audience・期限まで検証し、`email_verified` と**案内画面が発行する
  署名付き専用 cookie のログイン CSRF token**（header との double-submit。有効な cookie は使い回し＝favicon 等の
  自動リクエストや別タブの再描画で回転させず、表示中ページの token と cookie の食い違いによるログイン不能を防ぐ。
  `/favicon.ico` は公開アセット配信・Sec-Fetch-Mode≠navigate のサブリソース要求は案内へ redirect せず 401）を
  確認して署名付き session を作る。Cloud Run IAP は URL 到達前に強制転送するため
...
- **初回ログインのデフォルト seed ＋「データを初期化」（2026-07-12）**：新規ユーザーが初見で全タブ（書類を作る／
  育てる／クラス・園児／書類を見る）を体感できるよう、初回ログインで新規 workspace にデフォルト seed を自動投入する。
  3責務の線で配線：**データ＋投入＝harness**（`demo_seed_data.py`＝旧 scripts/seed_documents.py・seed_children.py の
  データ部を昇格〔物語時刻＝FY2026 7月中旬・仮名10人・クラス2・書類167件〈日誌138＝平日毎日3ヶ月分・経過記録20＝前年度〉・
  UNAPPROVED・INCOMPLETE_DATES〕／`demo_seed.py`＝`validate_all`・`seed_workspace`〔冪等・型成立かつ UNAPPROVED 外を承認済み・
  評価未記入の直近日誌は保存のみ〕・`reset_workspace`。`record_store` に `workspace_created` フラグ＋`purge_workspace_data`
  〔User/Workspace/利用枠は残す・消去順は削除依頼処理と `_purge_workspace_rows` を共用〕）／**トリガ・中継＝web**
  （`workspace.provision_user`＝/auth/google・/api/config・actor 解決の touch_user を一本化し workspace_created で単発
  発火・失敗はログイン本流を壊さない／`POST /api/account/reset`＝署名付き session 必須・reset_workspace の中継）／
  **UI**（ヘッダ「データ削除」→**「データを初期化する」**＝即時初期化・ログイン継続。**一回限りの開始導線ではなく
  毎回使える**（2026-07-12・デバッグ/検証で繰り返しサンプル状態へ戻す用途を優先）。アカウント削除受付〔30日〕は API 温存・
  UI からは外し privacy.html は問い合わせベースに整合）。「育てる」タブの指針/表記は行不在＝ローカルシード読取なので
  seed 投入不要（初期化は workspace 行の削除でシードへ戻る）。scripts は demo_seed の薄いラッパへ縮退（二重管理しない）。
  決定論テスト＝`tests/test_harness/test_demo_seed.py`（型成立・投入内容・冪等・境界・purge/reset）・`test_web.py`
  （403 ゲート・単発発火・実投入・初期化で復元＋session 継続。既存テストは auto-seed をスタブして件数前提を維持）。
- **書類管理タブの対象児フィルタ（2026-07-12）**：保育日誌・クラス月案はクラス全体（複数児）の記録が1書類に
  同居し、documents.child_id（単一FK・主対象児専用の書類だけが持つ）では引けないため、中を開かないと特定児の
  記述と分からなかった（根本要件＝ある子どもに関連する資料を横断的に見つける）。
  3責務の線で配線：**索引の永続化＝harness**（`documents.mentioned_children`＝JSON配列の表示名リスト。migration 0014
  で追加＋既存行を現行版から back-fill。`save_document` が保存のたびに既存の `_mentioned_children()`（従来は児童
  マスタの auto-create にしか使っていなかった）の出力で同期＝検索キーの列昇格という record_store.py の既存方針の
  自然な拡張。射影テーブルは作らない・本文JSONがSSOTのまま）／**API＝中継のみ**（`_doc_view` が `children` フィールド
  として返す・`/api/records` のレスポンス形の追加のみでエンドポイント変更なし）／**UI＝クライアント側フィルタ**
  （`records.js`＝「書類を見る」タブ上部に対象児入力〔`/api/children` 全件を datalist候補＝卒園児・未登場の子も
  含む〕。部分一致で `children` を絞り込み、既存の種別→子どもツリーをそのまま絞る（該当種別のみ表示・自動展開）。
  メタ一覧は既にタブオープン時に全件取得済みのためサーバ往復なし）。
  自由文検索（本文の部分一致）は今回のスコープ外（対象児で探すのが主要ユースケースに直結するため優先度を下げた）。
  決定論テスト＝`tests/test_harness/test_record_store.py`（複数児・単一児・編集による索引の同期を検証）。
...
- **層A の実デプロイ／eval ゲートCI の有効化**：GCP 側の **WIF（鍵レス）設定＋リポジトリ変数**
  （deploy＝`DEPLOY_SA`、eval＝`EVAL_SA`、共通＝`WIF_PROVIDER`/`GCP_PROJECT_ID` 等・ハードニングは
  `RUNTIME_SA`/`DATABASE_URL_SECRET`/`MAX_INSTANCES`＝`docs/ライブ実行手順.md`「本番運用ハードニング」）が前提。
  deployの任意設定は降格可能だが、evalの必須設定はskipせず赤にする。
  **DB 自動 migration を効かせるなら `CLOUDSQL_INSTANCE` 設定＋`DEPLOY_SA` に `roles/cloudsql.client`＋DB URL secret への
  ...
  eval ゲートCI は `infra/iam.tf` の専用 `eval-runner` SA＋`EVAL_SA` variable＋`google-adk[eval]`が前提。
  未設定・IAM不足・採点不能・baseline mean=nullはいずれもstrictで赤。初回はmain相当を
  `run_gate.py --update-baseline` で完全採点し、そのbaseline変更をレビューして確立する。
