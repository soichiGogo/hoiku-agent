...
# improve_session_id -> (runner, adk_session_id, workspace_id)。resume は同じ workspace だけ許可する。
_SESSIONS: dict[str, tuple[Any, str, str | None]] = {}
...
def register_improver_route(app: FastAPI) -> None:
    ...
    async def improve(req: ImproveRequest, request: Request) -> StreamingResponse:
        from google.adk.runners import InMemoryRunner

        workspace_id = resolve_workspace_id(request, datetime.now())
        book_id = f"workspace:{workspace_id}" if workspace_id is not None else None
        runner = InMemoryRunner(agent=build_improver_agent(book_id=book_id), app_name=_APP_NAME)
        ...
        _SESSIONS[improve_sid] = (runner, session.id, workspace_id)
    ...
    async def improve_resume(req: ImproveResumeRequest, request: Request) -> StreamingResponse:
        ...
        runner, adk_session_id, workspace_id = entry
        request_workspace_id = resolve_workspace_id(request, datetime.now())
