...
- **初回ログインのデフォルト seed ＋「データを初期化して始める」（2026-07-12・v2＝日誌を毎日3ヶ月分に拡充）**: 新規ユーザーの
  ...
  記入導線デモ）は未承認。ヘッダの**「データを初期化して始める」**（旧「データ削除」置換・`POST /api/account/reset`）は workspace の
  データ＋指針/表記カスタムを即時消去→seed 再投入（ログイン継続・`purge_workspace_data` は User/Workspace/利用枠を残す）。
  成功後は workspace 単位の完了印をブラウザへ保存し、同じブラウザでは再表示しない一回限りの開始導線。
