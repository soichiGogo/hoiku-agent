...
## 参照ポリシーの agentic 取得（2026-07-11）

育つ指針の PolicyBook に `kind=reference_policy` カードを scope ごと実質1枚置き、既定の参照 source を構造化管理する。InstructionProvider は guideline と有効な source を前置するが参照本文は固定注入しない。author が列挙引数だけの `fetch_reference(source)` で session state に seed 済みの候補を取得し、reviewer は `reference_manifest` で取得実績を検証して必要なら再取得する。

pipeline 常設の digest prep は撤去し `[authoring_loop → finalize]` とした。digest 化とクラス月案の児童別未反映境界は `harness/reference.py` から既存 aggregate/record_store を呼ぶ決定的実装に残す。Web/scripts の seed API・state key・workspace/PII 境界は不変。育てる UI は source on/off とメモを version 楽観ロック付きで直接編集するほか、improver が自然言語の修正メモを構造化案へ変換し、`ask_caregiver` の確認後だけ `policy_store.update_reference_policy` で即反映する。main の確立済み baseline は維持し、本変更後の母集団はマージ後に strict・完全採点で意図的に再採点して通常PRで更新する。

improver の PolicyBook は呼び出し側で束縛する。Web は署名付き session から共有 `web/workspace.py` で解決した `workspace:{workspace_id}`、CLI は default を使い、`build_policy_tools` のクロージャへ閉じ込める。book_id はLLMのツール引数にせず、Webの保留セッション再開時も開始時workspaceとの一致を検査する（認可境界）。
...
| ② 中身の決定・作成AI（§6） | 4種の author＝単一 `LlmAgent`＋tools。InstructionProvider は guideline と reference_policy の有効 source を提示し、author/reviewer が `fetch_reference` で候補本文を選択取得する | 情報収集・質問生成・「姿→ねらい/評価」変換・参照取得結果の要約。保育経過記録・保育要録は開示前提の表現も担う | Agentic |
| ② レビューAI（§7） | `agents/review_agent.py`（`LlmAgent`・月案/クラス月案/保育経過記録/保育要録で共用） | 別視点で点検（開示前提の表現観点含む）・APPROVED まで巡回（制御は harness） | Agentic |
| ③ 改善エージェント（§8） | `improver/`（別エントリ・手動起動） | 修正メモ→guideline の追加/改訂または reference_policy の変更を自走提案し、`ask_caregiver` を挟んで保育士の決定後だけ**即反映**（番人＝意味的競合精査＋保育士決定） | Agentic |
...
improver 固有: `improver/tools.py`（guideline の `read_policy_cards`／`propose_policy_card`＋意味的競合の申告＋完全重複ガード／
`commit_policy_card`、reference_policy の `read_reference_policy`／`propose_reference_update`／
`commit_reference_update`＝いずれも `ask_caregiver` の保育士決定後に即反映・`policy_store` を呼ぶ薄いラッパ）。
...
- **育つ指針＝構造化カード（§8 v1）**：`policy_store`（決定的 CRUD/render/完全重複ガード/履歴＝「回した証拠」・decided_by 含む・**scope＝共通/保育日誌/月案/保育経過記録/保育要録**）。`improver` は guideline の `read_policy_cards`→`propose_policy_card`→`ask_caregiver`→`commit_policy_card` と、reference_policy の `read_reference_policy`→`propose_reference_update`→`ask_caregiver`→`commit_reference_update` を使い分け、いずれも保育士決定後だけ即反映する。eval は取り込みから decouple（CI 専用）。
...
- **指針と参照既定の提示（§5）**：InstructionProvider は scope に応じた guideline と reference_policy の
  有効 source を提示する。参照本文は author/reviewer が fetch_reference で取得し、manifest を共有する。
