...
| ② 中身の決定・作成AI（§6） | `agents/author_agent.py`（日誌）/ `agents/monthly_author_agent.py`（月案）/ `agents/child_record_author_agent.py`（児童票・§19）＝単一 `LlmAgent`＋tools。instruction は `agents/instructions.py` の InstructionProvider で**文書作成指針（scope 別・共通＋当該書類）＋集積（前月/期間）を prompt 冒頭へ決定的に前置注入**（§5・harness の `render_for_doc`/`format_digest_for_prompt` を呼ぶ薄い組み立て） | 情報収集（Agentic RAG）・質問生成・「姿→ねらい/評価」変換・集積（前月/期間）の要約。児童票は**開示前提の肯定的・非断定的表現**も担う | Agentic |
...
- **保育士向け配布 UI（`web/`・B-full）**：`server.py` が `register_web_ui(app)` で `get_fast_api_app` に同居させる。
  保育士 SPA＝`/app/`（`/` も着地）、dev UI＝`/dev-ui/`、自前 API＝`/api/*`。SPA は**4タブ**＝**書類を作る**（日誌/月案/児童票を
  ...
  `GET /api/records/{id}`＝`record_store.get_document`・読取なので非ゲート・未接続/不在は正直に降格。前月・期間の集計など参照データの点検にも使える）。日誌/月案/児童票はフロントが ADK ネイティブ REST
