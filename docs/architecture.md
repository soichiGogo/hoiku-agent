...
- **外部リソース接続**: Gemini/Vertex は**接続済み**（ADC＋`GOOGLE_CLOUD_PROJECT`/`GEMINI_MODEL`＝author/reviewer は
  実 LLM でローカル稼働可）。**未接続**は Vertex RAG corpus と Agent Engine Memory Bank（`RAG_CORPUS`/`AGENT_ENGINE_ID`
  未設定＝`search_guideline`/`get_child_memory` は降格中。§9・config 設定で活性化）。
