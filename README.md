...
HOIKUAGENT は Cloud Run 上で UI・API・AI エージェントを稼働し、Cloud SQL に書類や園の設定を保存します。生成・検索・長期記憶は Vertex AI の各機能を使い、GitHub Actions から Artifact Registry を介して鍵レスで配信します。子どもの長期記憶への書き戻しは、**保育士の明示承認と型の検査を通った場合だけ**行います。
