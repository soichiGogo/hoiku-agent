...
# 推論を含む（制度用語と断定しない＝§10 / Certainty）。本製品では園の実様式に合わせ、
# 年度4期（各3か月）の期間を harness が決定的に保証する。
...
class ChildRecord(BaseModel):
    ...
    ここは型のみ）。期の区切りは園の実様式に合わせ、年度4期（4〜6月／7〜9月／10〜12月／
    1〜3月）の各3か月で固定する。成立判定の実体は harness.child_record_period に置く。
    """

    period: str = Field(description="対象期間（年度4期・各3か月。例: 2026-04〜2026-06）")
    age_band: AgeBand
    child_id: str  # 架空児のみ。実名は書かない（§14）
