...
```
src/hoiku_agent/
├── agent.py            … ルートエージェント（root_agent）＝doc_type 分岐ルータ（日誌/個別月案/クラス月案/児童票/保育要録・既定 日誌）
...
├── schemas/            … 書類スキーマ（日誌/個別月案/クラス月案/児童票/保育要録）・指針カード（policy）・年齢分岐・10の姿タグ（pydantic 集約）
├── web/                … 層A 配布UI（保育士 SPA /app/）：日誌/クラス月案/児童票/保育要録は ADK REST 直駆動・確認は園の実様式ルック（クラス月案＝区分×領域グリッド）・園の帳票PDF出力（chohyo_pdf）・園の実 Word 様式への流し込み出力（docx_fill＝templates/*.docx）・指針を育てる（improver）は SSE 中継・表記ルール辞書（notation.js＝/api/notation の CRUD）・書類アップロード取込（upload_extract＝PDF/Word/Excel を format 変換＋upload_parse＝抽出AIで既存スキーマへ・/api/parse-upload・「書類を見る」タブから）
