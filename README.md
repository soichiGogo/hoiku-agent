...
書類の「何をどの欄にどんな順番で書くか」は、作成手順として `harness/` で参照できるようにします。`agents/` 層では、手順書を元にエージェントが自身で必要な情報を判断し、ツールを用いて収集します。また `improver/` 層では、保育士の修正や指摘を収集し、普遍的かどうか、既存の指針と矛盾しないかを判断した上で指針カードの追加・改訂を提案します。
