...
- **保育日誌の手入力化＋AI 生成の退役＋クラス概念（2026-07-06・ヒアリング反映）**：日誌は「AI が観察メモから
  ...
  `children.class_id`・migration 0007・CRUD＝`list_classes`/`upsert_class`/`assign_child_to_class`/
  `list_children_in_class`・web `/api/classes`・新タブ「クラス・園児」＝`classes.js`）。② **日誌手入力フォーム**
  ...
  seed するため影響なし（`recall_child_history` だけ影響）。**校正AI（入力後の日本語チェック・言い換え提案）は
  Phase 3 で追加予定**（残）。
