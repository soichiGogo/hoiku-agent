...
1. 前提確認（**環境要件＝ここを外すと大半の UC が降格して試せない**。2026-07-15 の試行で確定）：
   - **worktree に `.env` をコピー**（primary checkout から。gitignore 済み・commit 禁止）＋ `uv sync --extra dev`。
   - `gcloud auth application-default login`（ADC）。**実 LLM は課金・利用枠（llm_budget）を消費**する。
   - **DATABASE_URL（ローカル Postgres）＋ seed 投入が事実上必須**：書類作成/アーカイブ/クラス・園児/初期化 等の
     content 系 UC は seed 記録が無いと**設計どおり生成をブロック**する（`app.js showMissingRecords`＝
     「作成に必要な記録を確認してください」。これはバグでなく正しい降格）。手順：
     `docker run` で Postgres を起こす → `.env` に `DATABASE_URL` → `uv run alembic upgrade head` →
     workspace に demo_seed を投入（`harness/demo_seed`。auto-seed は Google ログイン契機なので、ローカルは
     CLI/スクリプト経路で入れる）。DB を立てない場合は **降格でも試せる UC のみ**（下記）に絞る。
   - **ローカルは Google Sign-In ゲートが無効**（`SESSION_SECRET`/OAuth 未設定→SPA に直着地）。
     UC-24（ログイン）等の認証 UC は**ローカルでは試せない**＝デプロイ済み環境が要る（fail-warn で明記）。
...
2. `uvicorn server:app` を起動（バックグラウンド）。ブラウザ操作は **agent-browser CLI**（Bash）で行う：
   ...
   - `http://localhost:8000/app/` を開き、初回は「データを初期化する」でデフォルト seed を入れて
     全タブが体感できる状態にする（UC-25/26）。認証が要れば Google Sign-In を通す（UC-24）。
...
- `uv run --extra dev pytest`（決定論・関連 `tests/test_e2e/` 含む）＋ `ruff check .` / `ruff format .`。
