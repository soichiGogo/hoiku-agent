...
- **初回ログインのデフォルト seed ＋「データを初期化」（2026-07-12）**: 新規ユーザーの workspace に
  `harness/demo_seed(_data)` がクラス2（ひよこ組/あおぞら組）＋仮名名簿30人＋確定書類チェーン（日誌12/クラス月案6/
  保育経過記録9/要録1＝FY2026 通年・大半は承認済み・直近日誌2/最新クラス月案/要録は未承認＝承認フロー体感用）を
  冪等に自動投入（トリガ＝`touch_user` の `workspace_created`・`web/workspace.py` の `provision_user`）＝初見で全タブが
  体感できる。ヘッダの**「データを初期化」**（旧「データ削除」置換・`POST /api/account/reset`）は workspace のデータ＋
  指針/表記カスタムを即時消去→seed 再投入（ログイン継続・`purge_workspace_data` は User/Workspace/利用枠を残す）。
