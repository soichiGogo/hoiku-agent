...
| ② 中身の決定・作成AI（§6） | `agents/author_agent.py`（日誌）/ `agents/monthly_author_agent.py`（個別月案）/ `agents/class_monthly_author_agent.py`（クラス月案・園の実様式・§18）/ `agents/child_record_author_agent.py`（児童票・§19）/ `agents/nursery_record_author_agent.py`（保育要録・§19・L4）＝単一 `LlmAgent`＋tools。instruction は `agents/instructions.py` の InstructionProvider で**文書作成指針（scope 別・共通＋当該書類）＋集積（前月/期間/最終年度）を prompt 冒頭へ決定的に前置注入**（§5・harness の `render_for_doc`/`format_digest_for_prompt`／要録は `format_record_digest_for_prompt` を呼ぶ薄い組み立て） | 情報収集（Agentic RAG）・質問生成・「姿→ねらい/評価」変換・集積（前月/期間/最終年度）の要約。児童票・保育要録は**開示前提の肯定的・非断定的表現**も担う（要録は小学校引継ぎ＝10の姿の活用・最終年度に至るまでの育ちを recall_child_history から叙述） | Agentic |
| ② レビューAI（§7） | `agents/review_agent.py`（`LlmAgent`・日誌/月案/児童票/保育要録で共用） | 別視点で点検（開示前提の表現観点含む）・APPROVED まで巡回（制御は harness） | Agentic |
...
| 配信UI（層A・§11） | `web/`（`routes.py`・`improver_stream.py`・`chohyo_pdf.py`・`fonts/`・`static/`＝`docflow.js`/`docedit.js`/`policy.js`/`notation.js` 等） | 保育士向け配布 UI（`/app/`）。**4つ目の責務ではない presentation**：日誌/月案/児童票は ADK ネイティブ REST を直接駆動（自前 Runner なし）、確定下書きは**標準様式の見た目の編集フォーム**（`docedit.js`）で保育士が自由に編集→ `/api/finalize-edit` で harness が再検査・再整形。**現場でそのまま綴じる最終形＝園の帳票PDF**は `/api/export-pdf`（`chohyo_pdf.py`＝ReportLab・IPAex 埋め込み・確認印欄（担任/主任/園長）付き・描画のみ）。**Word 編集版＝園の実 Word 様式に流し込んだ .docx** は `/api/export-docx`（`docx_fill.py`＝python-docx・雛形は `web/templates/`・純pip・docx→PDF 変換はしない・対応 kind＝児童票/月案/保育要録）。改善エージェント（指針を育てる＝`policy.js`）だけ SSE 中継。**表記ルール辞書（`notation.js`＝`/api/notation` の CRUD）**は保育士が表記辞書を追加/編集/削除（harness 中継のみ・書込はゲート）。**アップロード取込（`records.js`＝「書類を見る」タブ）**は既存ファイル（PDF/Word/Excel）を種別フォルダから取り込み、`/api/parse-upload`（`upload_extract`＝format 変換＋`upload_parse`＝抽出AI駆動→`finalize_entry`）で既存スキーマ entry に解析→`docedit.js` で確認・修正→`/api/records`（`author_kind="imported"`）保存＝**以後 L2/L3/L4 seed として参照される** | 中継・描画 |

## harness 内訳（§5 物理マッピング）

| ファイル | 関数 | 役割 |
|---|---|---|
| `harness/router.py` | `DocTypeRouter` / `build_root_agent` | `state["doc_type"]` で日誌/月案/クラス月案/児童票/保育要録パイプラインを振り分ける決定的分岐（root_agent の実体・既定＝保育日誌＝§3/§18/§19） |
| `harness/pipeline.py` | `build_document_pipeline` / `build_authoring_loop` / `ApprovalGate` / `FinalizeAgent`(kind) / `is_approved` / `persist_visit_to_memory`(+`_should_persist_visit`) / `mark_caregiver_approved`(+`CAREGIVER_APPROVAL_KEY`) | 日誌：authoring_loop（[author→reviewer→ApprovalGate] を巡回・NEEDS_REVISION で author が再作成・APPROVED 早期終了）→ finalize の順序制御（文書作成指針は author/reviewer の InstructionProvider＝`agents/instructions.py` が prompt 冒頭へ前置注入するので pipeline に prep 段は無い＝§5）。FinalizeAgent は `final_document`（整形テキスト）に加え **`final_entry`（構造化エントリ dict）＋`final_doc_kind`** も state に残す（編集UIが欄ごとの編集フォームに描く）。`kind` は diary/monthly/class_monthly/child_record/nursery_record を切替。`after_agent_callback`＝**保育士の明示承認＋型成立**のときのみ来園を Memory Bank へ書き戻す（真の承認ゲート＝§9/§13） |
| `harness/monthly.py` | `DigestPrepAgent`（旧 MonthlyPrepAgent を入出力キーで一般化・**state-only イベント**） / `build_monthly_pipeline` | 月案：前月日誌を child_id 別に決定的集計（L2 還流）→ state["prev_month_digest"] に **content なしの state-only イベント**で載せる（前月集積は monthly_author の InstructionProvider が prompt へ整形注入・§10。content を持たせないのは eval judge が非LLM先頭段を採点不能にするのを避けるため＝§12）→ authoring_loop（日誌と共用・再作成）→ finalize(kind="monthly")。`DigestPrepAgent` は児童票（L3）・クラス月案（L2）と共用 |
| `harness/class_monthly.py` | `build_class_monthly_pipeline` | クラス月案（園の実様式＝月間指導計画・§18）：`DigestPrepAgent`（class_month_prep・個別月案と同じ prev_month_entries→prev_month_digest）でクラスの前月日誌を集計（L2 還流）→ クラス月案 author の authoring_loop（共用）→ finalize(kind="class_monthly")。個別月案（1児）と別 doc_type＝**クラス全体（年齢帯）単位**。指針 scope は月案を流用。区分×領域グリッド（養護2本柱＋教育5領域）は 0–2/3–5 共通で3つの視点分岐を適用しない（様式忠実） |
...
| `harness/youroku.py` | `RecordDigestPrepAgent` / `build_nursery_record_pipeline` | 保育要録（§19・L4）：最終年度の児童票（state["record_entries"]）を `RecordDigestPrepAgent`（record_prep）で決定的集計（L4 還流＝日誌でなく**児童票**を集める・実体は aggregate.child_record_digest）→ state["record_digest"] → 要録 author の authoring_loop（共用）→ finalize(kind="nursery_record") |
| `harness/schema_check.py` | `validate_fields` / `validate_monthly_fields` / `validate_class_monthly_fields` / `validate_child_record_fields` / `validate_nursery_record_fields`(+`_required_tag_type`) | 必須欄＋年齢分岐（0–2＝3つの視点 / 3–5＝5領域）。日誌/月案/児童票/保育要録で分岐の実体を共用（要録は年長＝5領域固定）。日誌の生活記録必須は **0–2 のみ**（3–5 は任意＝全年齢対応・§19）。**クラス月案は例外**＝様式が全年齢で5領域グリッドのため3つの視点分岐を課さず、グリッド各行のねらい＋0–2 の個人目標（≥1）を検査（§18） |
| `harness/draft.py` | `write_draft` / `write_monthly_draft` / `write_class_monthly_draft` / `write_child_record_draft` / `write_nursery_record_draft` ＋本文レンダラ（`_render_body`/`_render_section`/`_format_tagged_item`） | pydantic（DiaryEntry/MonthlyPlan/ClassMonthlyPlan/ChildRecord/NurseryRecord）→ **標準様式テキスト**へ整形。クラス月案は非線形の構造様式（児童票マトリクスと同様 template_store は通さず GRID_ROWS を歩いて描く＝§18）。**本文レイアウト（章立て＝セクションの順序・見出しラベル・種別・出し分け）は `template_store` の様式テンプレート（データ）を歩いて描く**（§18＝園差をテンプレ編集で吸収）。ヘッダ合成と個別記録ブロック・生活記録・出欠サマリ等の構造描画はコード。順序＝日誌:本日のねらい→出欠→主な活動→個別の記録（姿＋生活記録＝0–2 常時/3–5 記入時のみ）→…、月案:**養護2本柱→教育**、児童票:発達の経過→配慮特記→家庭連携→総合所見→次期、要録:最終年度の重点→個人の重点→保育の展開→特に配慮すべき事項→最終年度に至るまでの育ち（§19）。10の姿/3つの視点/5領域タグ明示 |
| `harness/template_store.py` | `load_template` / `find_template` / `book_view` / `load_book`/`load_book_meta`/`save_book` / `store_status` | **様式テンプレート＝本文レイアウトの宣言的データ**（`schemas/template.py` の DocTemplate/Section・閉じた種別語彙 text_block/text_inline/attendance/individual_notes/tagged_list/evaluation2）のストア。**3レンダラ共通で読む**＝テキスト整形（draft.py）・帳票PDF（chohyo_pdf の線形様式）・編集フォーム（docedit.js・`/api/doc-template`＝`book_view`）が本文セクションの順序/ラベルを取る（レイアウトの三重管理を解消・§18）。レイアウトのデータのみ（validation は持たない＝型の保証は schema_check・§5）。置き場は notation_store と同型＝明示 path ＞ `DATABASE_URL`（`template_books` 1行 JSONB・version 楽観ロック・migration 0005）＞ ローカル `knowledge/様式テンプレート.json`（git はシード）。**DB 到達不能／テーブル未整備（migration 0005 未適用）等の DB 障害は同梱シードへ降格して読む**（fail-loud だと全書類の write_*→finalize が落ちる＝本番 observed。レイアウトは常にシード代替可＝§5「降格safe」。`store_status` は DB を直接叩いて到達性を正直表示＝偽の "persistent" を出さない）。編集 UI は現状スコープ外（園差の実需で後続）。児童票の帳票PDF は年間マトリクス様式（線形でない）ため対象外 |
...
| `harness/aggregate.py` | `aggregate_by_child` / `prev_month_digest` / `format_digest_for_prompt`(label) / `child_record_digest` / `format_record_digest_for_prompt`(label) | 日誌集積（child_id 別）の state 用 digest・人間可読テキスト。`format_digest_for_prompt` は author/reviewer の InstructionProvider（agents/instructions.py）が state の digest を prompt へ整形するのに使う（月案 L2＝前月／児童票 L3＝期間・label で見出し切替）。**要録（L4）は日誌でなく最終年度の児童票を集積**＝`child_record_digest`（期順・領域頻度・発達叙述・総合所見）＋`format_record_digest_for_prompt`（InstructionProvider が record_digest を prompt へ整形）。要約生成は各 author |
| `harness/policy_store.py` | `load_book`/`load_book_meta`/`save_book` / `add_card`/`supersede_card`/`remove_card` / `render_to_text`（全再生・UI/eval）/ `render_for_doc`（共通＋当該 scope のみ・履歴なし＝前置注入用） / `find_exact_duplicate` / `card_view`/`history_view`/`book_view` / `store_status` | 育つ指針＝構造化カードストアの決定的 CRUD・完全重複ガード（安全網）・履歴・テキスト再生・API view（**scope＝共通/保育日誌/月案/児童票/保育要録**＝§19 で児童票・保育要録を追加・render/view マップに相乗り）。**指針編集の決定的実体はここに1つ**（improver/tools は薄いラッパ）。前置注入は `render_for_doc`（agents/instructions.py の InstructionProvider が呼ぶ）・全体可視化は `render_to_text`（UI `/api/policy`）。clock は外部注入。置き場は IO 節で解決＝明示 path ＞ `DATABASE_URL`（**Cloud SQL＝書類アーカイブと同じ DB・Phase 2 で GCS から統合**。`policy_books` 1行に book 丸ごと JSONB＝カードを行へ射影しない。`load_book_meta` の version → `save_book(if_version=…)` の compare-and-swap で read-modify-write を楽観ロック・競合は fail-loud・行不在はローカルシードを返し version 0＝create-only） ＞ ローカル `knowledge/文書作成指針.json`（git はシード）。**「回した証拠」＝カード内蔵の変更履歴（decided_by 含む）** |
| `harness/db.py` | `Base` / `engine` / `reset_engine_cache` / `database_url` | harness 共通の DB 接続基盤（engine キャッシュ・Declarative Base・JSONB variant）。record_store と policy_store が同じ `DATABASE_URL` を共有＝ドメインロジックは置かない |
| `harness/record_store.py` | `save_document` / `approve_document` / `list_documents` / `get_document`（単一書類の現行版全文＝閲覧タブ）/ `list_diary_entries` / `list_child_record_entries`（児童票の閲覧＝年間マトリクス過去期／**要録 L4 の seed 取得口**にも流用）/ `list_children` / `upsert_child` / `get_child`（表示名→本名/性別＝氏名欄解決）/ `touch_user` / `list_audit_events` / `store_status` ＋純関数 `honorific_for`/`compose_display_name`/`official_full_name` | 書類アーカイブ＝確定書類・児童マスタ・監査証跡（Cloud SQL PostgreSQL・Phase 1）。`DOC_KINDS＝diary/monthly/class_monthly/child_record/nursery_record`（要録は既存 documents テーブルに相乗り＝target_period 列に年度・クラス月案は target_month＝月をキーにクラス単位で相乗り・主対象児なし＝日誌と同型・migration 不要）。本文 JSON（PG は JSONB）が SSOT・検索キーだけ列昇格（射影テーブルなし）・版管理（`author_kind`＝AI 確定`ai`/保育士編集`caregiver`/**アップロード取込`imported`** を区別＝audit action は finalize/edit/**import**）・承認証跡（actor 自己申告）。**LLM もパイプラインも呼ばない**（フロント→web API→ここの明示フロー）。`DATABASE_URL` 未設定は降格。表示名→children.id（UUID）解決の唯一の境界。**児童マスタは名前の3要素を分けて持つ**＝`display_name`（呼び名＋敬称＝child_id 同定キー・不変）／`given_name`（名＝呼び名）＋`gender`（male/female＝敬称導出・男→くん/女→ちゃん固定・合成は `compose_display_name`）／`family_name`（姓＝氏名欄用の本名・**AI 非生成・DB のみ§14**）。氏名欄（要録/児童票 PDF）は `official_full_name`（姓＋名）で描く（migration 0006・既存行は末尾敬称から back-fill＝PG 限定）。スキーマ適用＝Alembic（repo root `migrations/`・`uv run alembic upgrade head`） |
...
child_record_author（児童票）＝`recall_child_history`・`search_guideline`・`ask_caregiver`
...
観察メモ（音声/手入力）＋ state["doc_type"]（既定＝保育日誌）
  └─ harness: DocTypeRouter (root_agent) … doc_type で日誌/月案/児童票パイプラインを決定的に振り分け（§10/§19）
       ...
       │    │    ※ 文書作成指針は author/reviewer の InstructionProvider（agents/instructions.py）が
       │    │      scope=保育日誌で共通＋当該書類の勘所を prompt 冒頭へ前置注入（render_for_doc・ストア障害は降格＝§5）
       │    ├─ authoring_loop (LoopAgent: author→reviewer→approval_gate を最大3巡)
       │    │    ├─ author (LlmAgent)  … 冒頭に前置された指針を与件に、不足は ask_caregiver(HITL) / RAG・記録・子メモリを収集し
       ...
       │                              表記正規化（notation_store・叙述系限定・降格safe）→
       ...
       │
       └─[クラス月案]─ class_monthly_pipeline (SequentialAgent・§18・園の実様式)  ※ 指針(scope=月案)＋前月集積は InstructionProvider が前置
            ├─ class_month_prep (DigestPrepAgent) … 前月のクラス日誌（state["prev_month_entries"]・複数児）を child_id 別に集計（L2 還流）
            │                              → state["prev_month_digest"]（個別月案と同じ既定キー・content 無し state-only）
       ...
       │
       └─[児童票]─ child_record_pipeline (SequentialAgent・§19)  ※ 指針＋期間集積は InstructionProvider が prompt 冒頭へ前置
       ...
       │
       └─[保育要録]─ nursery_record_pipeline (SequentialAgent・§19・L4・年長のみ)
            ├─ record_prep (RecordDigestPrepAgent) … 最終年度の児童票（state["record_entries"]）を child_id 別に
            │                              集計（L4 還流＝日誌でなく児童票を集める・aggregate.child_record_digest）
            │                              → state["record_digest"]＋集計テキストを提示
            ├─ authoring_loop （共用: nursery_record_author→reviewer→approval_gate を巡回・再作成）
            │    └─ nursery_record_author (LlmAgent) … 最終年度集積＋子メモリ（入所時から）を「保育の展開/個人の重点/
            │                                        最終年度に至るまでの育ち」へ再構成（小学校引継ぎ＝開示前提・10の姿）→ state["draft"]
...
保育要録（保育所児童保育要録・L4 還流＝最終年度の児童票の集積・年長のみ）**・**全年齢（0–2/3–5・要録は年長=5領域）**
（§3「日誌先行 → 月案は集積に乗せる」＋ §19「ヒアリング反映 2026-07＝主戦場を蓄積の下流再構成へ・
集積階層 日誌→月案（L2）→児童票（期・L3）→要録（年・L4）＝全段実装済み」）。
実装済み（決定的部分はテスト済み・GCP/LLM 非依存で稼働）:
- **doc_type 分岐 ＋ 月案パス ＋ L2 還流**：`DocTypeRouter`（root_agent）が doc_type で日誌/月案/児童票/保育要録を振り分け、
...
- **クラス月案パス（園の実様式＝月間指導計画・L2 還流・§18）**：園が実際に使うクラス月案フォーム
  ...
  個別月案（1児）と別 doc_type＝**クラス全体（年齢帯）単位**で、`DigestPrepAgent`（class_month_prep・個別月案と
  同じ prev_month_entries→prev_month_digest）が前月のクラス日誌を集計→ `class_monthly_author`（クラス全体の
  ねらい・区分×領域グリッド・0–2 は登場児ぶんの個人目標を生成／評価系欄は月末記入＝AI 非生成）→
  finalize(kind="class_monthly")。0–2/3–5 とも様式は5領域グリッドで共通＝3つの視点分岐は課さない（様式忠実）。
  ...
  バックエンド・アーカイブ閲覧で温存）。クラス月案の evalset 追加・アップロード取込対応は残課題（現場データ待ち）。
...
- **保育要録パス ＋ L4 還流（§19・集積階層の最終段）**：`NurseryRecord` スキーマ（全国統一様式の「保育に関する
  ...
  L4 の集積は **日誌でなく最終年度の児童票**を集める（`RecordDigestPrepAgent`（record_prep・`record_entries`→
  ...
  （最終年度の児童票を seed＝アーカイブ `list_child_record_entries` 接続時は取得・未接続はサンプル降格）。年度絞り込みの
  厳密化・複数年（入所〜前年度）の児童票供給は残課題（§18 と同枠・現状は最終年度分＋Memory Bank 参照）。
...
- **育つ指針＝構造化カード（§8 v1）**：`policy_store`（決定的 CRUD/render/完全重複ガード/履歴＝「回した証拠」・decided_by 含む・**scope＝共通/保育日誌/月案/児童票**）。`improver` は4ツール（`read_policy_cards`→`propose_policy_card`＝意味的競合の申告→`ask_caregiver`＝比較相談→`commit_policy_card`＝保育士決定で即反映）。eval は取り込みから decouple（CI 専用）。
...
- **児童票 scope の相乗り（§19・2026-07-05）**：`PolicyScope.児童票` を追加し、児童票作成の暗黙知（開示前提の肯定的・非断定的表現 等）を作成AI・レビューAI が参照し（harness が前置注入）保育士が improver UI で育てられるようにした。新ツールは作らず既存の指針カード機構（render/view/improver）を拡張＝二重実装しない。seed に児童票カード1枚。
...
- **prep の state-only 化＝eval 互換（§12・2026-07-05）**：ADK eval の rubric judge は invocation の**先頭 content イベント**の著者の developer instructions を引き、LLM 段のみを登録するため、非LLM の prep（`DigestPrepAgent`）が content 付きイベントを先頭に置くと採点不能になる（`policy_prep`/`period_prep not found`）。これが原因で**児童票 evalset 6件は追加時（2026-07-04）から採点できていなかった**（baseline は日誌16件で採点済み・child_record 追加後は未採点）。指針を InstructionProvider へ、集積を **content 無しの state-only イベント**へ移し、先頭 content 段＝author に保つことで日誌16＋児童票6の全 22 ケースが採点可能になった（`state["*_digest"]` は UI/scripts/tests 用に維持）。
...
- **eval ケース**：`eval/cases/diary_0_2.evalset.json` 16 件 ＋ `eval/cases/child_record.evalset.json` 6 件 ＋ `eval/cases/nursery_record.evalset.json` 3 件（§19・L4・session_input で doc_type/最終年度の児童票を seed・小学校引継ぎ＝開示前提の参照ドラフト）＝計 25 件（実在しない仮名ロスターのみ・現場の多様な状況）。rubric（3軸）は doc_type 非依存で全ケースを採点する。
...
- **本番運用ハードニング（2026-07-05・Google Cloud ベストプラクティス突合せ）**：Cloud Run 配信のセキュリティ/コスト MUST を
  ...
  **認証ポリシーの再現**（`IAP_AUDIENCE` var 設定時は `--iap`＋`--no-allow-unauthenticated` で IAP ゲート維持／未設定は
  `--allow-unauthenticated`）・**env 保全**（`--set-env-vars` は全置換ゆえ `DEMO_PASSCODE`＝GH secret／`MODEL_LOCATION`＝
  var 既定 global を明示管理し、再デプロイでゲート・生成 location が落ちないようにする）。
...
- **保育士向け配布 UI（`web/`・B-full）**：`server.py` が `register_web_ui(app)` で `get_fast_api_app` に同居させる。
  保育士 SPA＝`/app/`（`/` も着地）、dev UI＝`/dev-ui/`、自前 API＝`/api/*`。SPA は**上位3タブ**＝**書類を作る**（日誌/月案/児童票/保育要録を
  種別セグメント（`app.js` の `DOC_TYPES`）で1タブに統合＝フロー本体は共通で入力欄と seed だけ切替・対象児コンボは共有・結果エリアは
  種別ごとに保持・生成中は種別切替をロック＝`DocTypeRouter` の doc_type 分岐と 1:1）／**育てる**（＝2サブタブ 指針を育てる｜表記ルール＝
  ...
  **表示に必要な分だけ読む**＝折りたたみ既定で展開したフォルダの DOM のみ都度生成・重い本文はファイルを開いた分だけ取得。読取なので非ゲート・未接続/不在は正直に降格。前月・期間の集計など参照データの点検にも使える）。日誌/月案/児童票/保育要録はフロントが ADK ネイティブ REST
  （`/run_sse`・session 作成で月案/児童票/要録の seed・`PATCH` 承認・`function_response` で HITL 再開）を直接駆動（自前 Runner なし＝§9。要録は年度指定＋最終年度の児童票 seed＝`GET /api/records/child-record-entries`）。
  ...
  **現場でそのまま綴じる最終形＝園の帳票PDF**：確定/編集後の `final_entry` を「帳票PDFをダウンロード」で保存できる（`/api/export-pdf`→
  `chohyo_pdf.render_pdf`＝ReportLab で A4 罫線帳票・日誌/月案/児童票/保育要録・欄順は標準様式に一致・**描画のみで型検査は harness**）。**末尾に確認印欄
...
- **IAP 認証の土台（Phase 3 着手・2026-07-05）**：`web/iap.py`（`verified_iap_email`＝`IAP_AUDIENCE` 設定時のみ
  ...
  routes の actor 解決（検証済み email ＞ 自己申告・`/api/config` に user_email）。**dev サービス（hoiku-agent・
  us-central1）で IAP 有効化済み**（invoker は IAP サービスエージェントのみ＝Google アカウント認証で保護・`IAP_AUDIENCE`
  env 設定済み）。**CD（`deploy.yml`）はこの IAP ゲートを維持する**＝`IAP_AUDIENCE` var 設定時に `--iap`＋
  `--no-allow-unauthenticated`（公開しない）で再デプロイし、未設定環境は従来どおり公開＋パスコード（`--allow-unauthenticated`）。
  DEMO_PASSCODE は IAP 環境でも二重防御として維持（`deploy.yml` が GH secret から管理・下記ハードニング②）。決定論テストは
...
- **書類アップロード取込（「書類を見る」タブ・2026-07-06）**：既存の保育書類（**PDF/Word(.docx)/Excel(.xlsx)**）を
  取り込み、LLM で既存スキーマ（DiaryEntry/MonthlyPlan/ChildRecord/NurseryRecord）へ書き起こして、**アプリ生成物と
  同じように次の書類作成で参照できる**ようにする（唯一の新規能力＝ファイル→entry の抽出。以降は既存資産を再利用）。
  ...
  `POST /api/parse-upload`（multipart・**LLM を回すので `_GATED_PREFIX` でパスコードゲート**・未対応形式/種別 400・
  creds 無/LLM 失敗は 200＋parse_error で正直に降格）。UI は**ファイルシステム的**＝4種別フォルダを常時表示し、フォルダ
  （＋personal 種別の子フォルダ）を開くと先頭に「取り込む」行が出て kind〔＋child〕が場所で決まる（`records.js`）→
  ...
  `tests/test_harness/test_record_store.py`（imported→audit import）。実 LLM ＋実ブラウザ E2E（日誌/児童票の取込→保存→
  下流 seed 参照）を検証済み。
