...
このファイルは作成パイプラインの**共用機構**（`build_authoring_loop` / `ApprovalGate` / `FinalizeAgent` /
`persist_visit_to_memory`）を組む。**保育日誌の AI 生成パイプライン（旧 `build_document_pipeline`）は退役**
...
`finalize_entry`）。共用機構は月案/クラス月案/保育経過記録/保育要録の各パイプラインが使う:
    authoring_loop（[author → reviewer → ApprovalGate] を最大 N 巡）
      ...
      → finalize（確定 validate_*/write_* を末尾で決定的実行・HITL 承認フラグを立てる）
...
# `FinalizeAgent`・`persist_visit_to_memory`）は月案/クラス月案/保育経過記録/保育要録の各パイプライン
