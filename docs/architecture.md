...
| `harness/demo_seed.py`＋`demo_seed_data.py` | `validate_all` / `seed_workspace` / `reset_workspace`／データ部＝`ROSTER`（仮名10人）・`CLASSES`・`GRADUATE`・書類167件・`UNAPPROVED`・`INCOMPLETE_DATES` | **デモ用デフォルト seed**（実在しない仮名のみ・§14・物語時刻＝FY2026 の7月中旬）。初回ログイン（`web/workspace.py` の `provision_user`）で新規 workspace へ、クラス2（ひよこ組=0-2／あおぞら組=3-5・在籍児は誕生日→`roster_age_band` で割当）＋名簿10人＋確定書類チェーン計167件（**保育日誌138＝2026-04-01〜07-10 の平日毎日×2クラス・週テーマ×子ども別観察プールの決定的ローテ合成・age_months は生年月日から機械計算**／クラス月案8／**保育経過記録20＝前年度 FY2025〔「前回まで」還流と年度跨ぎ集積の素・今年度Ⅰ期はあえて未作成＝溜まった日誌から作るデモの主戦場〕**／要録1＝卒園児）を冪等投入（種別×児×期間で既存スキップ）。`UNAPPROVED`（最新クラス月案）と`INCOMPLETE_DATES`（直近2日の日誌＝評価未記入の記入導線デモ・validate 除外＋承認せず finalized）だけ未承認で残し承認フローを体感できる。「データを初期化」（web `/api/account/reset`）＝`reset_workspace`（purge→再 seed・ログイン継続）。CLI（`scripts/seed_children.py`・`scripts/seed_documents.py`）は薄いラッパ。投入は `finalize_entry`→`save_document`/`approve_document` の正規経路（Memory Bank へは書かない・未接続 skipped 降格）。整合は `tests/test_harness/test_demo_seed.py` が検算 |
