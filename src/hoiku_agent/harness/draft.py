...
児童票＝発達の経過→配慮特記→家庭連携→総合所見→次期、要録＝最終年度の重点→個人の重点→保育の展開→
...
def _format_tagged_item(note, item_field: str) -> str:
    # 枠組みタグ付きの叙述1件（月案の教育＝aim／児童票・要録の発達の経過＝description）を明示タグ付きで出す。
    # 枠組み（3つの視点/5領域/10の姿）を明示して出力する（§13）。月案/児童票/要録で共用（旧
...
def write_child_record_draft(record: ChildRecord, template_ref: str | None = None) -> str:
    """児童票（期ごとの保育経過記録）ドラフトを標準様式テキストへ整形して返す（§19・本文はテンプレ駆動）。
    ...
    lines = [
        f"■ 児童票・保育経過記録（{record.age_band.value} 歳児）"
