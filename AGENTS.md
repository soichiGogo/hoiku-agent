...
- **最終的な正は Obsidian vault `google-cloud-hackathon` の `設計/プロダクト方針.md`（製品方針）と
  `設計/エージェント設計.md`（アーキ）**。リポジトリ外にあり Codex からは読めない。
...
- ローカル実行: `adk run src/hoiku_agent`（CLI 対話）/ `adk web src`（開発 UI。agents dir＝`src/`・`/dev-ui/`）。
  **保育士向け配布 UI は `uvicorn server:app` → `http://localhost:8000/app/`**（日誌/月案/回す を1枚で・`web/`＝層A
  ...
  Google Sign-In と LLM 利用枠で LLM を回す口を保護＝コスト/濫用対策。規約は `src/hoiku_agent/web/AGENTS.md`）。
...
- **稼働範囲**: 保育日誌 ＋ **個別月案（L2 還流）** ＋ **クラス月案（園の実様式＝月間指導計画・L2 還流・§18）**
  ...
  エンティティ**（`record_store.Class`＋`children.class_id`・migration 0007/0009・**Class は組名＋年度だけを保持し、
  年齢帯は在籍児の生年月日と対象年度から導出**・web 新タブ「クラス・園児」＝園の名簿管理・日誌 roster の素）。
  **クラス月案**は個別月案（1児単位）と別の doc_type＝**文書の年齢帯単位**の園の実様式（区分×領域グリッド
...
- **実装済み（コード）**: レビュー APPROVED 早期終了（`ApprovalGate`/`is_approved`）/ 確定処理（`FinalizeAgent(kind)`＋
  `harness/finalize.py`・日誌/月案/保育経過記録）/ **月案パス＋L2 還流**（`DigestPrepAgent`→`prev_month_digest`→`monthly_author`）/
  **保育経過記録パス＋L3 還流**（`DigestPrepAgent`（period_prep）→`period_digest`→`child_record_author`・開示前提の表現指針＝§19）/
  **保育要録パス＋L4 還流**（`RecordDigestPrepAgent`（record_prep）＝それまでの保育経過記録すべてを `aggregate.child_record_digest` で集計→
  ...
  `/api/records/approve` がDB現行版を読み、`approved_memory_facts`→同期成功→approved の順で実行。
  生成パイプラインのcallbackでは書かない・手入力日誌も同じ経路・§9/§13）/
...
- **保育士向け配布 UI（`web/`・B-full）**: `/app/` の保育士 SPA＝**上位4タブ**（**書類を作る**（日誌/月案/保育経過記録/保育要録を**カテゴリ別グループ表示の種別メニュー**
  ...
  **育てる**（＝2サブタブ **指針を育てる**｜**表記ルール**。
  ...
  保育士が欄ごとに自由に編集**でき、保存時 `/api/finalize-edit`（harness `finalize_entry` 中継）で再検査・再整形→
  `/api/records/approve` でMemory Bank同期＋版単位の承認。タグ語彙は `/api/form-meta`（schemas Enum が SSOT）。
  **現場でそのまま綴じる最終形＝園の帳票PDF**は
  ...
  **その場（インライン）に改善エージェントを展開**（`makePolicy` を再インスタンス化＝`/api/improve` の `feedback` を実値化・別エントリ維持・
  ...
  実機検証済み（creds 有・gemini-2.5-pro＋Memory Bank）／非LLM面は `tests/test_web.py`。規約は `web/AGENTS.md`。
...
- **Google Sign-In（Phase 3）**: `/` は案内画面、Google 公式ボタンの redirect POST は `web/auth.py` が
  ID token（署名/audience/期限/email_verified）＋double-submit CSRF を検証し、署名付き session を作る。`/app/`・
  API・ADK 口は未ログインで fail-closed、Cloud Run IAP は使わない。`users.google_subject`（migration 0010）が
  Google の不変 `sub` を正として auto-provision・email 変更を追随し、actor は検証済み session ＞ 自己申告。
  **データ境界＝`Workspace`（migration 0011）**：Google user ごとの個人 workspace を作り、書類・園児・クラス・
  フィードバック・アーカイブ/集積 read を `workspace_id` で必ず絞る。既存データの所有者は推測せず隔離し、
  `/api/account/deletion-request` は本人確認済み session から30日後の削除依頼を受け付け、運営者が
  `uv run python scripts/process_deletion_requests.py` で期限到来分を消去する。公開ページは `/privacy`・`/terms`。
- **書類アーカイブ（Phase 1・本番運用ブラッシュアップ 2026-07）**: `harness/record_store`＝Cloud SQL PostgreSQL
  ...
  **書類フィードバック（`Feedback`＝👍👎＋ひとこと・migration 0008・`save_feedback`/`list_feedback`）は独立テーブル**＝確定/承認画面から
  ...
  確定/編集/承認を web（`/api/records` 系＋担当者名＝actor 自己申告）から版管理つきで永続化する。
  **Memory同期済み版＝`documents.memory_synced_version_id`/`memory_synced_at`（migration 0013）**として
  記録し、同じ版の再承認は二重投入せず、編集でcurrent versionが進むと未同期へ戻る。**L2/L3 の seed は
  アーカイブから自動取得**（scripts はローカル実行用サンプルへ降格可。保育士 web は参照記録を架空データで補わず生成を止める）。`DATABASE_URL` 未設定は降格。
...
- 補足: AGENTS.md は強制でなく文脈。PII 非コミットを確実化するなら `PreToolUse` hook が本筋（将来 `.Codex/` で）。

# ブランチ・コミット・PR

グローバル AGENTS.md のブランチ戦略・コミット/PR 規約に従う（ここでは再定義しない）。
