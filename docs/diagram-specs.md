...
2. **つくる流れ**: harness が参照候補を安全に用意 → InstructionProvider の前置指示と取得ツール群から作成 AI へコンテキストを注入 → 作成 AI が必要情報を統合して変換 → レビュー AI が参照実績も点検 → harness の確定前検査 → 保育士の確認・編集・承認。
3. **育てる流れ**: 図上は右から左へ、修正・評価 → 改善エージェント → 指針カード提案 → 保育士の決定 → 育つ指針。保育士からの入力線は右側、次の作成への還流線は左側に分け、交差させない。

作成 AI の直前には、`InstructionProvider`（育つ指針・参照ポリシー）と全 author 共通の4ツールを独立した入力パネルとして置く。実装上は `fetch_reference`（過去書類・名簿）、`recall_child_history`（子どもの長期記憶）、`search_guideline`（保育所保育指針・10の姿）、`ask_caregiver`（不足情報）に対応するが、図上は横文字のツール名を出さず、日本語の役割だけを大きく表示する。
