...
def class_roster_rows(roster: list) -> list[dict]:
    """有効な名簿行（呼び名のある dict）だけを返す（判定の実体はここに1つ）。

    reference の count（manifest の在籍児数）と本文整形が同じ母集合を見るための共用述語＝
    片方だけ条件を変えて「manifest は N 名・本文は M 行」の静かな不整合を作らない。
    """
    return [r for r in roster if isinstance(r, dict) and str(r.get("child_id") or "").strip()]
