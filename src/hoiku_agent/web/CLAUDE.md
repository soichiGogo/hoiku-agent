...
- `workspace.py` … 署名付き Google session の subject から `record_store.touch_user` を通じて workspace_id を
  ...
  ヘッダの**「データを初期化する」ボタン**（`app.js` の `#reset-data`・サインイン＋アーカイブ接続時のみ表示）は
  `POST /api/account/reset`＝`demo_seed.reset_workspace`（書類・園児・クラス・フィードバック・指針/表記カスタムを
  即時消去→seed 再投入・User/Workspace/利用枠は残しログイン継続）。**一回限りの開始導線ではなく毎回使える**
  （デバッグ・検証で workspace を繰り返しサンプル状態へ戻したい用途を優先＝2026-07-12）。**アカウント削除の受付
