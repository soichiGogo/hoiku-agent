...
- **実装済み（コード）**: レビュー APPROVED 早期終了（`ApprovalGate`/`is_approved`）/ 確定処理（`FinalizeAgent(kind)`＋
  ...
  **scope＝共通/保育日誌/月案/保育経過記録/保育要録**・improver は guideline の read→propose（意味的競合の申告）→ask（比較相談）→commit に加え、reference_policy も read→propose→ask→commit（保育士決定後だけ即反映）・eval は decouple）/
