...
| 育つ文書作成指針 | 構造化カード。runtime の正＝`POLICY_STORE_URI`（gs://）設定時 **GCS オブジェクト**（バージョニング推奨・Cloud Run でも永続）／未設定はローカル `knowledge/文書作成指針.json`（git はシード＋dev 証拠 commit） | agent は読み取り（`read_policy`＝`render_to_text`）／improver が**保育士の決定で即反映**（add/supersede・`policy_store`・GCS は generation 楽観ロック） |
...
- **児童票パス ＋ L3 還流（§19）**：`ChildRecord`/`DevelopmentNote` スキーマ（期・発達の経過＝領域別叙述・
  ...
  **帳票PDF は年間マトリクス様式（実様式準拠）**：A4 横・行＝領域（0–2:3視点/3–5:5領域＋その他）×列＝4期の
  年間1枚に、今回の期の列だけ埋めて他は空欄の罫線で出す（期ごと追記の運用＝手書き追記可）。身長・体重は
  原簿系の任意欄（`ChildRecord.height_cm/weight_kg`・**AI は生成しない**＝プロンプトで創作禁止・保育士が
  編集フォームで記入）。過去期の自動集約（書類アーカイブ）は残課題。
