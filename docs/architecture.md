...
| `harness/demo_seed.py`＋`demo_seed_data.py` | `validate_all` / `seed_workspace` / `reset_workspace`／データ部＝`ROSTER`（仮名30人）・`CLASSES`・書類28件・`UNAPPROVED` | **デモ用デフォルト seed**（実在しない仮名のみ・§14）。初回ログイン（`web/workspace.py` の `provision_user`）で新規 workspace へ、クラス2（ひよこ組=0-2／あおぞら組=3-5・在籍児は誕生日→`roster_age_band` で割当）＋名簿30人＋確定書類チェーン（日誌12/クラス月案6/保育経過記録9/要録1＝FY2026 通年・L2/L3/L4 の seed 集積が実際に効く）を冪等投入（種別×児×期間で既存スキップ）。`UNAPPROVED`（直近日誌2・最新クラス月案・要録）だけ未承認で残し承認フローを体感できる。「データを初期化」（web `/api/account/reset`）＝`reset_workspace`（purge→再 seed・ログイン継続）。CLI（`scripts/seed_children.py`・`scripts/seed_documents.py`）は薄いラッパ。投入は `finalize_entry`→`save_document`/`approve_document` の正規経路（Memory Bank へは書かない・未接続 skipped 降格） |
...
- **初回ログインのデフォルト seed ＋「データを初期化」（2026-07-12）**：新規ユーザーが初見で全タブ（書類を作る／
  ...
  データ部を昇格〔仮名30人・クラス2・書類28件・UNAPPROVED〕／`demo_seed.py`＝`validate_all`・`seed_workspace`〔冪等・
  UNAPPROVED 以外を承認済み〕・`reset_workspace`。`record_store` に `workspace_created` フラグ＋`purge_workspace_data`
