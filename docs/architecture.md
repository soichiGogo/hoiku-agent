...
| `harness/class_monthly.py` | `build_class_monthly_pipeline` | クラス月案（園の実様式＝月間指導計画・§18）：`DigestPrepAgent`（class_month_prep・個別月案と同じ prev_month_entries→prev_month_digest＋**`reflections_key="prev_month_reflections"` で前月の評価・反省も集める＝決定B**）でクラスの前月日誌を集計（L2 還流）→ クラス月案 author の authoring_loop（共用）→ finalize(kind="class_monthly")。個別月案（1児）と別 doc_type＝**クラス全体（年齢帯）単位**。指針 scope は月案を流用。区分×領域グリッド（養護2本柱＋教育5領域）は 0–2/3–5 共通で3つの視点分岐を適用しない（様式忠実） |
