...
class DocumentType(str, Enum):
    ...
    保育要録は §19 が予告した集積階層（日誌→月案→児童票→要録）の最終段（L4）＝最終年度（年長）の
    児童票を集積し小学校へ引き継ぐ書類。年一回・年長のみだが一人あたりの労力が最重量。
    """
...
# §19 が予告した集積階層（日誌→月案→児童票→要録）の最終段。全国統一様式（こども家庭庁）で、
...
# 点検はレビューAI／ここは型のみ）。最終年度の児童票の集積（L4 還流）に乗せて書く。欄名は自治体
...
class NurseryRecord(BaseModel):
    ...
    最終年度の児童票の集積（state["record_digest"]＝L4 還流）を「保育の展開と子どもの育ち」
    「個人の重点」へ流し、入所〜前年度の育ちは recall_child_history／過去年度の児童票から
    「最終年度に至るまでの育ち」へ叙述する。年長（5歳児）専用のため age_band は常に 三から五歳
    （＝5領域）。就学先・保育期間など「入所に関する記録」は原簿系＝AI 生成しない任意欄（児童票の
    ...
    development_notes: list[DevelopmentNote] = Field(
        ...
        "年齢分岐タグ必須＝年長は5領域・§19）。DevelopmentNote を児童票と共用"
    )
