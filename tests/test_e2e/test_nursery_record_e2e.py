...
設計コンテキスト §19（保育要録＝集積階層の最終段 L4＝最終年度の保育経過記録を集積）/ §16。保育経過記録の
...
def _child_record(period: str, domain_tag: str, desc: str) -> dict:
    """最終年度（年長=3–5）の保育経過記録（架空児A）の dict。L4 還流の入力。"""
    return {
        "period": period,
        "age_band": "3-5",
        "child_id": "架空児A",
...
def _author_text(record: dict) -> str:
    return (
        "最終年度の保育経過記録集積から保育要録の下書きを作成しました。\n```json\n"
...
def test_nursery_record_path_aggregates_final_year_and_finalizes():
    """① ルータ分岐（保育要録）＋② L4 還流（最終年度の保育経過記録集計）＋③ 要録確定。"""
