...
- ADK エージェントは **`build_xxx()` ファクトリ関数**で構築して返す（`build_author_agent` /
  `build_monthly_author_agent` / `build_child_record_author_agent` / `build_nursery_record_author_agent` /
  `build_review_agent` / `build_improver_agent` / `build_document_pipeline` / `build_monthly_pipeline` /
...
- **保育士向け配布 UI（`web/`・B-full）**: `/app/` の保育士 SPA＝**上位3タブ**（**書類を作る**（日誌/月案/児童票/保育要録を種別セグメントで統合＝
  ...
  **Word 編集版＝園の実 Word 様式に流し込んだ .docx** は「Word様式でダウンロード」→ `/api/export-docx`（`web/docx_fill.py`＝python-docx で
  `web/templates/*.docx` を埋める・純pip・docx→PDF のサーバ変換はしない・対応 kind＝児童票/月案/保育要録を `/api/config` の `docx_kinds` で出し分け）。
