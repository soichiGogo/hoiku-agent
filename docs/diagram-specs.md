...
作成 AI の直前には、`InstructionProvider`（育つ指針・参照ポリシー）と全 author 共通の4ツールを独立した入力パネルとして置く。ツールは `fetch_reference`（過去書類・名簿）、`recall_child_history`（子どもの長期記憶）、`search_guideline`（保育所保育指針・10の姿）、`ask_caregiver`（不足情報）とし、そこから作成 AI へコンテキストが注入されることを示す。
