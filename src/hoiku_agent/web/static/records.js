...
const UPLOAD_META = {
  ...
  child_record: { targetLabel: "対象期間", inputType: "period", placeholder: "", wantsChild: true, wantsAge: true },
...
export function makeRecords(ui) {
  ...
  function openUploadForm(kind, child) {
    ...
    let targetInput;
    if (meta.inputType === "period") {
      targetInput = el("select", "rup-input");
      for (const period of ui.childRecordPeriods || []) {
        const option = el("option");
        option.value = period.value;
        option.textContent = period.label;
        targetInput.appendChild(option);
      }
      if (ui.currentChildRecordPeriod) targetInput.value = ui.currentChildRecordPeriod;
    } else {
      targetInput = el("input", "rup-input");
