...
- 保育経過記録（doc_type=保育経過記録・L3 還流＝期間日誌の集積）は専用入口 `uv run python scripts/run_child_record.py
  --child-id はるとくん --period 2026-04〜2026-06`（要 LLM 資格情報。期間日誌を seed）。
- 保育要録（doc_type=保育要録・L4 還流＝最終年度の保育経過記録の集積・年長のみ）は専用入口
  `uv run python scripts/run_youroku.py --child-id はるとくん --fiscal-year 2026`（要 LLM 資格情報。
  最終年度の保育経過記録を seed。アーカイブ接続時は `list_child_record_entries` から取得・未接続はサンプル降格）。
...
- **稼働範囲**: 保育日誌 ＋ **個別月案（L2 還流）** ＋ **クラス月案（園の実様式＝月間指導計画・L2 還流・§18）**
  ...
  ＋ **保育要録（保育所児童保育要録・L4 還流＝最終年度の保育経過記録の集積・小学校引継ぎ・年長のみ＝§19）**。
  ...
  **クラス月案**は個別月案（1児単位）と別の doc_type＝**クラス全体（年齢帯）単位**の園の実様式（区分×領域グリッド
  ...
  集積階層は **日誌→月案（L2）→保育経過記録（期・L3）→要録（年・L4）**（§19・全段実装済み）。
- **実装済み（コード）**: レビュー APPROVED 早期終了（`ApprovalGate`/`is_approved`）/ 確定処理（`FinalizeAgent(kind)`＋
  ...
  **保育要録パス＋L4 還流**（`RecordDigestPrepAgent`（record_prep）＝最終年度の保育経過記録を `aggregate.child_record_digest` で集計→
  ...
  **eval ケース 25 件**（日誌16＋保育経過記録6＋保育要録3・実在しない仮名ロスターのみ・現場に即した内容）/ ツールの降格（RAG/Memory 未設定でも落ちない）。
...
- **保育士向け配布 UI（`web/`・B-full）**: `/app/` の保育士 SPA＝**上位3タブ**（**書類を作る**（日誌/月案/保育経過記録/保育要録を種別セグメントで統合＝
  ...
  期間日誌 seed・保育要録は年度指定＋最終年度の保育経過記録 seed＝`GET /api/records/child-record-entries`）。**確定下書きは標準様式の見た目の編集フォーム（`docedit.js`）で
