...
- **稼働範囲**: 保育日誌 ＋ **個別月案（L2 還流）** ＋ **クラス月案（園の実様式＝月間指導計画・L2 還流・§18）**
  ...
  一次情報の蓄積口。web の docedit→`finalize_entry`→アーカイブ＝`author_kind=caregiver`＝L2/L3/L4 seed）。AI 生成書類の
...
- **保育士向け配布 UI（`web/`・B-full）**: `/app/` の保育士 SPA＝**上位4タブ**（**書類を作る**（日誌/月案/保育経過記録/保育要録を種別セグメントで統合。
