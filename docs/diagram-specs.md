...
8. **Secret Manager**: `DATABASE_URL` を Cloud Run と DB migration へ安全に注入する。
9. **Cloud Logging / Trace**: 構造化ログ、リクエスト相関、エージェント実行軌跡。
10. **IAM / Service Accounts**: runtime / deploy / eval / tf-admin を用途別・最小権限で分離する。
11. **GitHub Actions / WIF**: GitHub OIDC を Google Cloud の短期認証へ交換し、鍵レスで用途別 SA を借用する。
12. **Cloud Build / Artifact Registry**: ソースからコンテナをビルドし、イメージを保存して Cloud Run へ配信する。
13. **Cloud SQL Auth Proxy / Alembic**: デプロイ前に DB schema を前進させる。
14. **Terraform / Cloud Storage**: プラットフォーム基盤を宣言管理し、GCS を Terraform state とビルド一時領域に使う。アプリデータは置かない。
...
- WIF / IAM → Cloud SQL Auth Proxy / Alembic → Cloud SQL: `デプロイ前 migration`。
