...
児童票（run_child_record.py）と対称。保育要録は**最終年度（年長）の児童票の集積（L4 還流）**を
seed する必要があるため、専用スクリプトで起こす（root_agent には組み込み済みだが、`adk web` は
doc_type と最終年度の児童票を seed しづらいので、デモ/検証はこの入口を使う）。

このスクリプトは:
1. 最終年度の児童票を読む。優先順位＝ `--entries-file`（JSON 配列）＞ 書類アーカイブ
   （`DATABASE_URL` 設定時・record_store から指定児の児童票を取得＝Phase 1）＞ 同梱の仮名サンプル（降格）。
...
最終年度の児童票の JSON は ChildRecord の配列（run_child_record.py の出力と同型）。
...
def _sample_record_entries(child_id: str) -> list[dict]:
    """最終年度（年長=3–5）の児童票サンプル（L4 還流のデモ入力）。実データは置かない＝仮名のみ（§14）。
...
def _archive_record_entries(child_id: str) -> list[dict]:
    """書類アーカイブから指定児の児童票を引く（L4 seed の本命経路・Phase 1）。

    最終年度分の児童票を集積の素にする。DATABASE_URL 未設定・該当なしは []＝呼び出し側がサンプルへ
...
def main() -> None:
    ...
    parser.add_argument(
        "--entries-file", help="最終年度の児童票（ChildRecord の JSON 配列）。無ければ同梱サンプル"
    )
    ...
    print(f"[seed] 最終年度の児童票 {len(record_entries)} 件（{seed_src}）")
