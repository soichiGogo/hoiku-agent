...
- **初回ログインのデフォルト seed ＋「データを初期化」（2026-07-12）**：新規ユーザーが初見で全タブ（書類を作る／
  ...
  **UI**（ヘッダ「データ削除」→**「データを初期化して始める」**＝即時初期化・ログイン継続。成功後は workspace 単位の
  完了印を `localStorage` に保存し、同じブラウザでは再表示しない一回限りの開始導線。アカウント削除受付〔30日〕は API 温存・
