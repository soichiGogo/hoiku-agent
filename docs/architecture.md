...
| `harness/pipeline.py` | `build_authoring_loop` / `ApprovalGate` / `FinalizeAgent`(kind) / `is_approved` | 作成パイプラインの**共用機構**（月案/クラス月案/保育経過記録/保育要録が使う）。authoring_loop（[author→reviewer→ApprovalGate] を巡回・NEEDS_REVISION で author が再作成・APPROVED 早期終了）→ finalize の順序制御。FinalizeAgent は `final_document`（整形テキスト）に加え **`final_entry`（構造化エントリ dict）＋`final_doc_kind`** も state に残す。生成終了時には人の承認がまだ無いためMemory Bankへは書かない。**旧 `build_document_pipeline`（保育日誌の AI 生成）は退役** |
| `harness/memory_writeback.py` | `approved_memory_facts` / `persist_approved_facts` | 保存済み承認版から計画欄を除いた**子どもの事実欄だけ**を子ども別factへ決定的に変換し、Memory Bankの既存像へconsolidateする。唯一の呼出元はWebの `/api/records/approve`。日誌も同じ経路。生成完了を待ってからDBをapprovedへ進める（接続済み失敗はfail-closed、未接続は降格） |
