...
保育経過記録（run_child_record.py）と対称。保育要録は**その児のそれまでの保育経過記録すべての集積
（L4 還流・全期・年度跨ぎ含む＝依存モデル 2026-07。日誌は足さない）**を seed する必要があるため、
専用スクリプトで起こす（root_agent には組み込み済みだが、`adk web` は doc_type と保育経過記録を
...
保育経過記録の JSON は ChildRecord の配列（run_child_record.py の出力と同型）。
...
def _sample_record_entries(child_id: str) -> list[dict]:
    ...
    年度4期（各3か月）にわたる年長1年間の育ちの推移（自己発揮→協同→就学期待）を含め、要録の
    ...
    periods = [
        ...
        },
        {
            ...
            "overall": "自信をもって表現し、仲間と役割を担う姿が育った",
            "next": "就学への期待を高めていく",
        },
        {
            ...
            "age_months": "6歳0か月",
            "development": [
                ("小学校見学を通して就学への期待を高め、見通しをもって準備した", "環境"),
                ("生活発表会で友だちと協力し、最後までやり遂げた", "表現"),
            ],
            "overall": "就学への期待を力に変え、自信をもって卒園を迎える姿が育った",
            "next": "小学校生活を楽しみに待つ",
        },
    ]
    return [
...
def _archive_record_entries(child_id: str) -> list[dict]:
    """書類アーカイブから指定児のそれまでの保育経過記録すべてを引く（L4 seed の本命経路・Phase 1）。

    依存モデル（2026-07 確定）＝「対応する児童の作成済み過去のものすべて・年度跨ぎ可」。全期を渡し、
    最終年度中心の再構成・入所時からの育ちの叙述は author の責務（prompts）。DATABASE_URL 未設定・
    該当なしは []＝呼び出し側がサンプルへ降格する。
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--entries-file",
        help="それまでの保育経過記録（ChildRecord の JSON 配列）。無ければ同梱サンプル",
    )
    ...
    print(f"[seed] それまでの保育経過記録 {len(record_entries)} 件（{seed_src}・全期）")
