...
- **実装済み（コード）**: レビュー APPROVED 早期終了（`ApprovalGate`/`is_approved`）/ 確定処理（`FinalizeAgent(kind)`＋
  ...
  **scope＝共通/保育日誌/月案/保育経過記録/保育要録**・improver は read→propose（意味的競合の申告）→ask（比較相談）→commit（保育士決定後だけ即反映）・eval は decouple）/
...
- **保育士向け配布 UI（`web/`・B-full）**: `/app/` の保育士 SPA＝**上位4タブ**（**書類を作る**（日誌/月案/保育経過記録/保育要録を**カテゴリ別グループ表示の種別メニュー**
  ...
  **育てる**（＝2サブタブ **指針を育てる**｜**表記ルール**。
  ...
  ＝`record_store.get_document`・読取なので非ゲート・未接続は正直に降格・参照データの点検にも使える。**閲覧だけでなくアーカイブ済み書類を編集・（再）承認できる**〔右ペイン「編集する」→`docedit.js`→`/api/finalize-edit`→`/api/records`〔`author_kind="caregiver"`・新版〕・「承認する」→`/api/records/approve`。**承認済みを編集すると承認は失効し finalized へ戻る**＝record_store が demote〕。外から特定書類を編集モードで開く `records.openDoc(id,{edit,focus})` を公開〔クラス月案作成時の「評価未記入の日誌へ飛んで記入」導線が使う＝下記〕。**対象児フィルタ（2026-07-12）**＝日誌・クラス月案はクラス全体（複数児）が1書類に同居し中を開かないと特定児の記述と分からないため、`documents.mentioned_children`（登場する子ども全員の表示名＝保存のたびに現行版へ同期する検索キーの列昇格・migration 0014）を部分一致でクライアント側フィルタし、既存の種別→子どもツリーをそのまま絞り込む（該当種別のみ表示・自動展開）。候補は `/api/children` 全件（卒園児・未登場の子も含む）。サーバ往復なし（メタ一覧は既に全件取得済み）。**取込**＝各種別フォルダ〔＋personal 種別の子フォルダ〕の「取り込む」から
  ...
  期間日誌＋前回までの記録 seed・保育要録は年度指定＋それまでの保育経過記録すべて seed＝`GET /api/records/child-record-entries`・クラス月案は seed 3系統＋在籍児名簿＝`GET /api/records/class-monthly-seed`）。**確定下書きは標準様式の見た目の編集フォーム（`docedit.js`）で
...
- **初回ログインのデフォルト seed ＋「データを初期化する」（2026-07-12・v2＝日誌を毎日3ヶ月分に拡充／同日デバッグ用に
  毎回使えるボタンへ変更）**: 新規ユーザーの
  workspace に `harness/demo_seed(_data)` がクラス2（ひよこ組/あおぞら組）＋仮名名簿10人（＋卒園児1人）＋確定書類
  チェーン計167件を冪等に自動投入（トリガ＝`touch_user` の `workspace_created`・`web/workspace.py` の `provision_user`）＝
  初見で全タブが体感できる。物語時刻は**FY2026 の7月中旬**＝**保育日誌 138件（2026-04-01〜07-10 の平日毎日×2クラス・
  毎日書くもの）**／クラス月案8／**保育経過記録20〔年度4期・各3か月の閉じた期間・前年度 FY2025＝「前回まで」還流の素・今年度第1期はあえて未作成＝
  溜まった日誌から作るデモの主戦場〕**／要録1〔卒園児〕。大半は承認済み・最新クラス月案と直近2日の日誌（評価未記入＝
  記入導線デモ）は未承認。ヘッダの**「データを初期化する」**（旧「データ削除」→「データを初期化して始める」から改称・
  `POST /api/account/reset`）は workspace の
  データ＋指針/表記カスタムを即時消去→seed 再投入（ログイン継続・`purge_workspace_data` は User/Workspace/利用枠を残す）。
  **一回限りの開始導線ではなく毎回使える**（localStorage の完了印チェックを撤去＝デバッグ/検証で繰り返しリセットしたい
  用途を優先・2026-07-12）。
  アカウント削除受付（30日）は API 温存・UI からは外し privacy.html は問い合わせベース。CLI の
  `scripts/seed_children.py`・`scripts/seed_documents.py` は demo_seed の薄いラッパに縮退（二重管理しない）。
