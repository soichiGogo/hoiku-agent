...
| ファイル | 関数 | 役割 |
|---|---|---|
| `harness/router.py` | `DocTypeRouter` / `build_root_agent` | `state["doc_type"]` で日誌/月案/クラス月案/保育経過記録/保育要録パイプラインを振り分ける決定的分岐（root_agent の実体・既定＝保育日誌＝§3/§18/§19） |
...
- **IAP 認証の土台（Phase 3 着手・2026-07-05）**：`web/iap.py`（`verified_iap_email`＝`IAP_AUDIENCE` 設定時のみ
  ...
  `record_store.users`／`touch_user`（検証済み email の auto-provision・migration 0002・認可は持たない）／`set_user_display_name`
  （自分の表示名の登録/編集）＋ routes の actor 解決（検証済み email ＞ 自己申告・`/api/config` に user_email／user_display_name）
  ＋**`POST /api/user`**（サインイン中ユーザーの表示名を **IAP 検証済み email に紐づけ**て設定＝email は body 由来を使わず偽装不可・
  未サインインは 403・IAP が認証ゆえパスコードゲート外）。**ヘッダの担当者欄はサインイン時「自分の表示名」編集欄**になり名前を表示/登録
  （未登録は email をプレースホルダに出して促す・未サインインは従来の自己申告＋localStorage）。**dev サービス（hoiku-agent・
  ...
  `tests/test_web.py`（偽装ヘッダ無視・検証成功で actor 優先・失敗で自己申告降格・**`/api/user`＝未認証403/保存が config・証跡に反映/非ゲート**）／
  `tests/test_harness/test_record_store.py`（touch_user・**set_user_display_name の provision/更新/clamp/降格**）。
