...
このファイルは作成パイプラインの**共用機構**（`build_authoring_loop` / `ApprovalGate` / `FinalizeAgent`）を
組む。**保育日誌の AI 生成パイプライン（旧 `build_document_pipeline`）は退役**
した（ヒアリング 2026-07：日誌は保育士が自分の言葉で打つ一次情報の蓄積口＝web の手入力フォーム→
`finalize_entry`）。共用機構は月案/クラス月案/保育経過記録/保育要録の各パイプラインが使う:
    authoring_loop（[author → reviewer → ApprovalGate] を最大 N 巡）
      ...
      → finalize（確定 validate_*/write_* を末尾で決定的実行・HITL 承認待ちフラグを立てる）

保育士の承認とMemory Bank書き戻しは、生成後のWeb操作順に合わせ `/api/records/approve` を唯一の入口に
する。パイプライン終了時にはまだ人の承認が無いため、ここから書き戻してはならない（§9/§13）。
...
スコープと関連パス:
- このファイルが組むのは作成パイプラインの**共用機構**のみ（保育日誌の AI 生成パイプラインは退役）。
- 各書類の候補 seed は state に保持し、author/reviewer が fetch_reference を呼ぶと
  `harness.reference` が既存 aggregate で決定的に集計する。`router.py` の `DocTypeRouter` は
  state["doc_type"] で各書類へ決定的に振り分ける（§3/§4/§10）。
...
def build_authoring_loop(
    ...
    author は書類ごとに差し替える（呼び出し側が build_monthly_author_agent / build_child_record_author_agent
    等を渡す）。保育日誌の作成AI は退役（手入力＝AI 生成を通さない）。
...
# 保育日誌の AI 生成パイプライン（旧 `build_document_pipeline`）は退役した（ヒアリング 2026-07：日誌は
# 保育士が自分の言葉で打つ一次情報の蓄積口で、AI 全生成は現場運用に合わない）。日誌は web の手入力フォーム
# （docedit）→ `finalize_entry` → アーカイブで作る＝AI を通さない。ここで組む共用機構（`build_authoring_loop`・
# `FinalizeAgent`）は月案/クラス月案/保育経過記録/保育要録の各パイプライン
# （monthly.py / class_monthly.py / child_record.py / youroku.py）が引き続き使う。
