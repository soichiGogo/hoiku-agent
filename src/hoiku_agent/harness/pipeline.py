...
パイプライン（root_agent の実体）:
    author（作成・Agentic RAG / 不足は ask_caregiver）→ state["draft"]
      ...
      → [after_agent_callback] persist_visit_to_memory（型成立の確定時のみ、来園セッションを
        その子の長期メモリ＝Agent Engine Memory Bank へ書き戻す＝§9/§13「来園のたびに像が育つ」）
...
def _should_persist_visit(state: Mapping) -> bool:
    """確定下書きが "型として成立" したか（来園を子の長期メモリへ書き戻してよいか・決定的）。

    設計コンテキスト §9/§13：0–2 個別＝子ども別長期メモリは「来園のたびに像が育つ」。書き戻しの
    "判定" は harness の決定的ロジック（純関数）として切り出し、LLM 非依存にテストできるようにする。
    finalize が parse 成功・違反0・整形出力ありで終えたときだけ True。型不成立の下書きで像を汚さない。

    注（v0 の近似）：真の「保育士が承認した」明示アクションのプリミティブは未実装のため、
    v0 は "型成立の確定下書き" を承認の代理トリガにする。承認アクションでゲートするのは次フェーズ
...
def build_document_pipeline(
    ...
    `after_agent_callback`＝`persist_visit_to_memory`：全段の後に1度、型成立の確定なら来園を
    Memory Bank へ書き戻す（§9/§13）。memory_service 未配線でも降格するため、注入の有無で
    順序制御や既存挙動は変わらない（書き戻し先が無ければ素通り）。
    """
