...
pipeline 常設の digest prep は撤去し `[authoring_loop → finalize]` とした。digest 化とクラス月案の児童別未反映境界は `harness/reference.py` から既存 aggregate/record_store を呼ぶ決定的実装に残す。Web/scripts の seed API・state key・workspace/PII 境界は不変。育てる UI は source on/off とメモを version 楽観ロック付きで直接編集する。eval baseline は再採点まで null。
