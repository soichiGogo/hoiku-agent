...
| `harness/template_store.py` | `load_template` / `find_template` / `load_book`/`load_book_meta`/`save_book` / `store_status` | **様式テンプレート＝本文レイアウトの宣言的データ**（`schemas/template.py` の DocTemplate/Section・閉じた種別語彙 text_block/text_inline/attendance/individual_notes/tagged_list/evaluation2）のストア。draft.py（後続で帳票PDF/編集フォーム）が `load_template(doc_type)` で読む。レイアウトのデータのみ（validation は持たない＝型の保証は schema_check・§5）。置き場は notation_store と同型＝明示 path ＞ `DATABASE_URL`（`template_books` 1行 JSONB・version 楽観ロック・migration 0005）＞ ローカル `knowledge/様式テンプレート.json`（git はシード）。編集 UI は現状スコープ外（園差の実需で後続） |
