...
- **保育士向け配布 UI（`web/`・B-full）**：`server.py` が `register_web_ui(app)` で `get_fast_api_app` に同居させる。
  保育士 SPA＝`/app/`（`/` も着地）、dev UI＝`/dev-ui/`、自前 API＝`/api/*`。SPA は**3タブ**＝**書類を作る**（日誌/月案/児童票を
  種別セグメント（`app.js` の `DOC_TYPES`）で1タブに統合＝フロー本体は共通で入力欄と seed だけ切替・対象児コンボは共有・結果エリアは
  種別ごとに保持・生成中は種別切替をロック＝`DocTypeRouter` の doc_type 分岐と 1:1）／指針を育てる／表記ルール。日誌/月案/児童票はフロントが ADK ネイティブ REST
