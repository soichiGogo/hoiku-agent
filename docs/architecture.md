...
| `harness/record_store.py` | `save_document` / `approve_document` / `list_documents` / `get_document`（単一書類の現行版全文＝閲覧タブ）/ `list_diary_entries` / `list_child_record_entries`（児童票の閲覧＝年間マトリクス過去期／**要録 L4 の seed 取得口**にも流用）/ `list_children` / `touch_user` / `list_audit_events` / `store_status` | 書類アーカイブ＝確定書類・児童マスタ・監査証跡（Cloud SQL PostgreSQL・Phase 1）。`DOC_KINDS＝diary/monthly/child_record/nursery_record`（要録は既存 documents テーブルに相乗り＝target_period 列に年度・migration 不要）。本文 JSON（PG は JSONB）が SSOT・検索キーだけ列昇格（射影テーブルなし）・版管理（AI 確定/保育士編集を区別）・承認証跡（actor 自己申告）。**LLM もパイプラインも呼ばない**（フロント→web API→ここの明示フロー）。`DATABASE_URL` 未設定は降格。表示名→children.id（UUID）解決の唯一の境界。スキーマ適用＝Alembic（repo root `migrations/`・`uv run alembic upgrade head`） |
...
- **保育士向け配布 UI（`web/`・B-full）**：`server.py` が `register_web_ui(app)` で `get_fast_api_app` に同居させる。
  保育士 SPA＝`/app/`（`/` も着地）、dev UI＝`/dev-ui/`、自前 API＝`/api/*`。SPA は**4タブ**＝**書類を作る**（日誌/月案/児童票/保育要録を
  種別セグメント（`app.js` の `DOC_TYPES`）で1タブに統合＝フロー本体は共通で入力欄と seed だけ切替・対象児コンボは共有・結果エリアは
  種別ごとに保持・生成中は種別切替をロック＝`DocTypeRouter` の doc_type 分岐と 1:1）／指針を育てる／表記ルール／**書類を見る**（アーカイブ閲覧＝
  `record_store` の確定書類を種別で絞り込み一覧→クリックで現行版の整形テキスト＋帳票PDF を確認＝`records.js`・`GET /api/records`／
  `GET /api/records/{id}`＝`record_store.get_document`・読取なので非ゲート・未接続/不在は正直に降格。前月・期間の集計など参照データの点検にも使える）。日誌/月案/児童票/保育要録はフロントが ADK ネイティブ REST
