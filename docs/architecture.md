...
| `harness/draft.py` | `write_draft` / `write_monthly_draft` / `write_child_record_draft` / `write_nursery_record_draft` | pydantic（DiaryEntry/MonthlyPlan/ChildRecord/NurseryRecord）→ **標準様式テキスト**へ整形（ネット調査で裏取りした章立て・順序。日誌＝ヘッダ（記録日・天候・気温/組の任意欄）→本日のねらい→主な活動→個別の記録（姿＋生活記録＝0–2 常時/3–5 記入時のみ）→…、月案＝**養護2本柱（生命の保持/情緒の安定）→教育**の順、児童票＝ヘッダ（期・対象児）→発達の経過（領域別叙述）→配慮・特記→家庭連携→総合所見→次期に向けて、要録＝ヘッダ（年度・対象児・就学先）→最終年度の重点→個人の重点→保育の展開と子どもの育ち（5領域/10の姿）→特に配慮すべき事項→最終年度に至るまでの育ち＝§19）。10の姿/3つの視点/5領域タグ明示 |
